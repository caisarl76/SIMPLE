...
def fixture_publication_authorization(
    ...
    *,
    ...
    prior_transition_or_entry = (
        ...
        else None if previous is None else previous["owner_transition"]
    )
    prior_transition = (
        None if prior_transition_or_entry is None
        else prior_transition_or_entry.get(
            "transition", prior_transition_or_entry
        )
    )
    ...
    provisional_authorization = encode_pending_publication_authorization(
        ...
        recovery_reason=(
            None if request["payload"]["recovery_generation"] is None
    ...
    if previous_active is None:
        ...
        previous_active_authorization = previous_owner[
    ...
    transition = encode_owner_transition(
        ...
        from_authorization_id=(
            None if previous_active_authorization is None
...
def _fixture_owner_mapping_for_authorization(
    ...
    *,
    ...
    if recovery_generation is None:
        ...
        owner["recovery_authorizations"] = [
            *owner["recovery_authorizations"], authorization,
        ]
    ...
    owner.update(
        {
            ...
            "active_authorization_sha256": authorization[
                "authorization_sha256"
            ],
            "authorization_sequence": authorization[
        ...
        *owner["recovery_results"],
        encode_recovery_result(
            ...
            previous_result=(
                ...
                if owner["recovery_results"] else None
            ),
        ),
    ]
    owner.update(
        {
            ...
            "authorization_sequence": transition[
...
class ForkedInstallerService:
                            ...
                            try:
                                ...
                                if response_status == "TRANSPORT_DROPPED":
                                    if request["operation"].startswith("install_"):
                                        ...
                                        unactivated = (
                                            fixture_unactivated_owner_record(request)
                                        )
                                        owner_record = (
    ...
    def _send_response(
        ...
        *,
        ...
        try:
            ...
            wire_faults = {
                "malformed_schema", "request_id_mismatch",
                "payload_identity_mismatch", "digest_encoding",
                "digest_length", "component_missing", "component_extra",
                "component_type", "root_identity_type",
                "root_identity_missing", "root_identity_extra", "root_mode",
                "success_error", "terminal_phase", "busy_error_missing",
                "busy_error_code", "busy_error_retryable_type",
                "busy_error_extra", "busy_mutation_field",
            }
            wire_payload = (
                ...
                if fault in wire_faults else payload
            )
            if wire_status in {
                ...
                ForkedInstallerService._record_terminal_response(
                    transaction_lock.parent,
                    request_id=request["request_id"],
                    operation_token_sha256=request["payload"][
                        "operation_token_sha256"
                    ],
                    payload=payload,
                    fault_hook=lambda name: os._exit(87)
                    if os.environ.get(
            ...
            body = (
                ...
                if fault == "partial_body" else wire_payload
            )
            ancillary = [] if not rights else [
                (socket.SOL_SOCKET, socket.SCM_RIGHTS, rights)
            ]
            connection.sendmsg(
                [struct.pack("!I", len(wire_payload)), body], ancillary
            )
            return publication
        finally:
        ...
        *,
        ...
        publication = {
            ...
            for key in (
                ...
                "final_tree_sha256", "root_identity", "receipt_sha256",
            )
        }
        published = (
            protected / "published" / publication["final_tree_sha256"]
        )
        ...
        if hashlib.sha256(completion).hexdigest() != publication[
            "completion_sha256"
        ]:
        ...
        receipt = (
            protected / "receipts"
            / f"{publication['final_tree_sha256']}.json"
        )
        ...
        ForkedInstallerService._validate_existing_receipt(
            ...
            expected_parent_identity=receipt_contract[
                "receipt_parent_identity"
            ],
            expected_uid=receipt_contract["input_uid"],
            expected_gid=receipt_contract["input_gid"],
        )
        if hashlib.sha256(receipt_bytes).hexdigest() != publication[
            "receipt_sha256"
        ]:
        ...
        stored_response, response_identity = (
            ForkedInstallerService._read_existing_install_replay_response(
                ...
                expected_operation_identity=records[0][
                    "response_operation_identity"
                ],
    ...
    def _read_existing_install_replay_response(
        ...
        *,
        ...
        protected = control_parent / "protected-installer"
        configured = ForkedInstallerService._configured_receipt_contract(
            control_parent
        )
        ...
        try:
            ...
            if tuple(record["phase"] for record in records) != (
                ...
                raise RuntimeError(
                    "INSTALLER_TEST_INSTALL_REPLAY_DIRECTORY_JOURNAL"
                )
            response_root_identity = ForkedInstallerService._fd_identity(
                response_root_fd
            )
            if (
                response_root_identity != configured["response_parent_identity"]
                or any(
            ...
            if (
                ...
                raise RuntimeError(
                    "INSTALLER_TEST_INSTALL_REPLAY_OPERATION_IDENTITY"
                )
    ...
    def _resume_protected_publication(
        ...
        *,
        ...
        publication: dict[str, object] = {
            "journal_path": os.fspath(journal),
            "response_operation_identity": records[0][
                "response_operation_identity"
            ],
            "source_parent_identity": ForkedInstallerService._path_identity(
                published_parent
        ...
        if _stable_publication_identity(
            response_operation_identity
        ...
        expected_receipt_parent_identity = receipt_contract[
            "receipt_parent_identity"
        ]
        receipt_sha256 = next(
            (
            ...
            try:
                ForkedInstallerService._validate_existing_receipt(
                    ...
                    expected_parent_identity=(
                        expected_receipt_parent_identity
                    ),
                    expected_uid=receipt_contract["input_uid"],
                    expected_gid=receipt_contract["input_gid"],
                )
        ...
        *,
        ...
        publication = {
            ...
            "source_parent_identity": {
                **records[0]["source_parent_identity"], "mode": 0o700,
            },
            "staging_identity": {
                **records[0]["staging_identity"], "mode": 0o700,
            },
        }
        actor_known = any(
            request["request_id"] == recovery_request["request_id"]
        ...
        staging_removal_already_pending = (
            phases[-1] == "STAGING_REMOVE_PENDING"
        )
    ...
    def _ensure_response_operation_directory(
        ...
        *,
        ...
        if (
            len(operation_token_sha256) != 64
            or any(
                character not in "0123456789abcdef"
                for character in operation_token_sha256
            )
        ):
        ...
        if records and records[0]["response_root_identity"] != (
            response_root_identity
        ):
            os.close(response_root_fd)
            raise RuntimeError(
                "INSTALLER_TEST_RESPONSE_DIRECTORY_ROOT_IDENTITY"
            )
        ...
        try:
            if not records:
                ...
                try:
                    ...
                    raise RuntimeError(
                        "INSTALLER_TEST_RESPONSE_DIRECTORY_ORPHAN"
                    )
            ...
            if (
                ...
                raise RuntimeError(
                    "INSTALLER_TEST_RESPONSE_DIRECTORY_IDENTITY"
                )
            return expected
        finally:
            os.close(response_root_fd)
    ...
    def _record_terminal_response(
        ...
        *,
        ...
        if (
            ...
            raise RuntimeError("INSTALLER_TEST_RESPONSE_OPERATION_TOKEN")
        response_root = control_parent / "protected-installer" / (
            "response-artifacts"
        )
        expected_identity = (
            ForkedInstallerService._ensure_response_operation_directory(
    ...
    def _configured_receipt_contract(
        ...
        if type(value) is not dict or set(value) != {
            "input_gid", "input_uid", "receipt_parent_identity",
            "response_parent_identity", "schema_version",
    ...
    def _validate_existing_receipt(
        ...
        *,
        ...
        try:
            ...
            if (
                parent_actual != expected_parent_identity
            ...
            while len(final_bytes) <= len(expected_bytes):
                chunk = os.read(
                    final_fd, len(expected_bytes) + 1 - len(final_bytes)
                )
                if not chunk:
                    break
...
def test_manager_fsyncs_transition_before_closing_handoff(
    ...
    reply = InstallerReply("INSTALLED", {}, None, "a" * 64, handoff_fd, b"{}")
...
from simple.eval_runtime.installer_client import (
    ...
    encode_pending_publication_authorization,
    encode_publication_authorization,
    read_protected_response_artifact,
    validate_protected_publication_contract,
    validate_owner_transition_against_records,
    validate_owner_transition_entry,
    validate_owner_history_for_service,
    validate_locked_response_history,
)
...
from .fakes import (
    ...
    fixture_publication_authorization,
    fixture_reactivation_transition_entry,
    fixture_unactivated_owner_record,
)
...
INSTALL_BEFORE_OWNER_RECORD = fixture_unactivated_owner_record(INSTALL_REQUEST)
INSTALL_OWNER_RECORD = fixture_owner_record_for_authorization(
    INSTALL_AUTHORIZATION,
    request=INSTALL_REQUEST,
    previous_owner_record=INSTALL_BEFORE_OWNER_RECORD,
)
RECOVERY_REQUEST = {
    **INSTALL_REQUEST,
...
RECOVERY_AUTHORIZATION = fixture_publication_authorization(
    ...
    previous_owner_record=INSTALL_OWNER_RECORD,
)
RECOVERY_OWNER_RECORD = fixture_owner_record_for_authorization(
    RECOVERY_AUTHORIZATION,
    request=RECOVERY_REQUEST,
    previous_owner_record=INSTALL_OWNER_RECORD,
)
INSTALL_TRANSITION_ENTRY = fixture_owner_transition_entry(
    INSTALL_AUTHORIZATION,
    previous_owner_record=INSTALL_BEFORE_OWNER_RECORD,
    target_owner_record=INSTALL_OWNER_RECORD,
    creates_authorization=True,
)
RECOVERY_TRANSITION_ENTRY = fixture_owner_transition_entry(
    RECOVERY_AUTHORIZATION,
    previous_owner_record=INSTALL_OWNER_RECORD,
    target_owner_record=RECOVERY_OWNER_RECORD,
    creates_authorization=True,
)
INSTALL_OWNER_TRANSITIONS = (INSTALL_TRANSITION_ENTRY,)
RECOVERY_OWNER_TRANSITIONS = (
    *INSTALL_OWNER_TRANSITIONS,
    RECOVERY_TRANSITION_ENTRY,
)


def _rehash_fixture_authorization(
    authorization: dict[str, object], request: dict[str, object]
) -> None:
    ...
    transition["transition_sha256"] = hashlib.sha256(
        json.dumps(
            transition_body, sort_keys=True, separators=(",", ":")
        ).encode()
    ).hexdigest()
    authorization_body = {
...
OWNER_TRANSITION_TRANSACTION_BOUNDARIES = tuple(
    f"{position}_{component}_{step}"
    for component in (
        "owner_transition_prepared",
        "owner_transition_history",
        "owner_transition_owner",
        "owner_transition_committed",
    )
    for step in ("replace", "file_fsync", "parent_fsync")
...
RECEIVE_TIME_ARTIFACT_BOUNDARIES = (
    "before_protected_response_artifact_validation",
    "after_protected_response_artifact_validation",
)
RESPONSE_ARTIFACT_PUBLICATION_BOUNDARIES = (
    "after_response_directory_pending",
    "after_response_directory_mkdir",
    "after_response_directory_parent_fsync",
    "after_response_directory_created",
    "before_response_temporary_create",
    "after_response_temporary_create",
    "during_response_temporary_write",
    "after_response_temporary_write",
    "after_response_temporary_data_fsync",
    "after_response_temporary_chmod",
    "after_response_temporary_metadata_fsync",
    "before_response_atomic_publish",
    "after_response_atomic_publish",
    "after_response_temporary_unlink",
    "after_response_parent_fsync",
)
FRESH_ATTEMPT_STATES = {
    "abort_received": (
        1, "BASE_METADATA_NORMALIZED", "active",
        "MIRROR_ABORTED_ATTEMPT",
    ),
    "old_attempt_aborted": (
        1, "BASE_METADATA_NORMALIZED", "aborted-before-prepared",
        "ALLOCATE_FRESH_BASE_ATTEMPT",
    ),
    "fresh_base_allocated": (
        2, "BASE_ALLOCATED", "active", "REBUILD_FRESH_BASE",
    ),
    "fresh_base_copied": (
        2, "BASE_COPIED", "active", "ACTIVATE_FRESH_INSTALL",
    ),
    "fresh_install_authorized": (
        2, "BASE_COPIED", "active", None,
    ),
}
FRESH_ATTEMPT_TRANSITIONS = (
    ("old_attempt_abort", "abort_received", "old_attempt_aborted"),
    ("fresh_base_allocate", "old_attempt_aborted", "fresh_base_allocated"),
    ("fresh_base_copy", "fresh_base_allocated", "fresh_base_copied"),
    (
        "fresh_install_authorize", "fresh_base_copied",
        "fresh_install_authorized",
    ),
)
FRESH_ATTEMPT_CRASH_BOUNDARIES = {}
for prefix, before_state, after_state in FRESH_ATTEMPT_TRANSITIONS:
    FRESH_ATTEMPT_CRASH_BOUNDARIES.update(
        {
            f"before_{prefix}": FRESH_ATTEMPT_STATES[before_state],
            f"before_{prefix}_owner_replace": (
                FRESH_ATTEMPT_STATES[before_state]
            ),
            f"after_{prefix}_owner_replace": FRESH_ATTEMPT_STATES[after_state],
            f"before_{prefix}_owner_fsync": FRESH_ATTEMPT_STATES[after_state],
            f"after_{prefix}_owner_fsync": FRESH_ATTEMPT_STATES[after_state],
            f"before_{prefix}_parent_fsync": FRESH_ATTEMPT_STATES[after_state],
            f"after_{prefix}_parent_fsync": FRESH_ATTEMPT_STATES[after_state],
            f"after_{prefix}": FRESH_ATTEMPT_STATES[after_state],
        }
    )


def _fixture_response_bytes(
    control_parent: Path, request: Mapping[str, object]
) -> bytes:
    request_id = str(request["request_id"])
    token = str(request["payload"]["operation_token_sha256"])
    basename = hashlib.sha256(request_id.encode()).hexdigest()
    return (
        control_parent / "protected-installer" / "response-artifacts"
        / token
        / f"{basename}.bin"
    ).read_bytes()


def _snapshot_protected_installer_tree(root: Path) -> dict[str, tuple[object, ...]]:
    protected = root / "protected-installer"
    ...
    for path in sorted(protected.rglob("*")):
        ...
        if path.is_symlink():
            snapshot[relative] = (
                "symlink", status.st_ino, stat.S_IMODE(status.st_mode),
                os.readlink(path),
            )
        elif path.is_dir():
            snapshot[relative] = (
                "directory", status.st_ino, stat.S_IMODE(status.st_mode),
            )
        else:
            payload = path.read_bytes()
            snapshot[relative] = (
                "file", status.st_ino, stat.S_IMODE(status.st_mode),
                hashlib.sha256(payload).hexdigest(), len(payload),
            )
    return snapshot


def _complete_install_replay_fixture(control_parent: Path) -> dict[str, object]:
    publication = ForkedInstallerService._materialize_protected_publication(
        ...
        recovery_requests=(),
        recovery_authorizations=(),
        owner_transition_history=INSTALL_OWNER_TRANSITIONS,
    )
    payload = ForkedInstallerService._encode_response(
        request=INSTALL_REQUEST,
        status="INSTALLED",
...
def test_install_replay_returns_exact_stored_bytes_without_mutation(tmp_path) -> None:
    _complete_install_replay_fixture(tmp_path)
    stored_before = _fixture_response_bytes(tmp_path, INSTALL_REQUEST)
    protected_before = _snapshot_protected_installer_tree(tmp_path)
    _, stored = ForkedInstallerService._replay_existing_install_publication(
        tmp_path,
        install_request=INSTALL_REQUEST,
        install_authorization=INSTALL_AUTHORIZATION,
        recovery_requests=(),
...
def _corrupt_install_replay_response_evidence(
    ...
    elif corruption in {"journal_missing", "journal_and_artifact_missing"}:
        journal = (
            control_parent / "protected-installer" / "journals"
            / f"{token}.jsonl"
        )
        journal.unlink()
        ForkedInstallerService._fsync_dir(journal.parent)
        if corruption == "journal_and_artifact_missing":
            artifact.unlink()
            ForkedInstallerService._fsync_dir(operation)
    else:
...
@pytest.mark.parametrize(
    "corruption, error",
    (
        ("missing", "INSTALLER_TEST_INSTALL_REPLAY_ARTIFACT_SET"),
        ("extra", "INSTALLER_TEST_INSTALL_REPLAY_ARTIFACT_SET"),
        ("mutated", "INSTALLER_TEST_INSTALL_REPLAY_ARTIFACT_BYTES"),
        (
            "pending_only",
            "INSTALLER_TEST_INSTALL_REPLAY_DIRECTORY_JOURNAL",
        ),
...
def test_install_replay_never_repairs_incomplete_response_evidence(
    corruption: str, error: str, tmp_path
) -> None:
    ...
    with pytest.raises(RuntimeError, match="INSTALLER_TRANSPORT_UNCERTAIN"):
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "journal_missing", "journal_and_artifact_missing",
    ),
)
...
def test_response_artifact_atomic_publication_recovers_every_boundary(
    boundary: str, tmp_path
) -> None:
    operation_token = "d" * 64
    response_root = tmp_path / "protected-installer" / "response-artifacts"
    response_root.mkdir(mode=0o700, parents=True)
    response_parent = response_root / operation_token
    sibling = response_root / ("e" * 64)
    sibling.write_bytes(b"preserve-me")
    sibling_inode = sibling.stat().st_ino
    payload = b'{"canonical":"complete-response"}'
    ...
    if pid == 0:
        ForkedInstallerService._record_terminal_response(
            ...
            operation_token_sha256=operation_token,
            payload=payload,
            fault_hook=lambda name: os._exit(84) if name == boundary else None,
        )
        os._exit(0)
    _, status = os.waitpid(pid, 0)
    assert os.waitstatus_to_exitcode(status) == 84
    ...
    basename = hashlib.sha256(b"atomic-response").hexdigest()
    final = response_parent / f"{basename}.bin"
    final_stat = final.stat(follow_symlinks=False)
    assert final.read_bytes() == payload
    assert stat.S_ISREG(final_stat.st_mode)
    assert final_stat.st_nlink == 1
    assert stat.S_IMODE(final_stat.st_mode) == 0o444
    assert not tuple(response_parent.glob(f".{basename}.*.pending"))
    directory_journal = (
        tmp_path / "protected-installer" / "response-directory-journals"
        / f"{operation_token}.jsonl"
    ).read_bytes()
    directory_records = decode_exact_response_directory_journal(
        directory_journal, operation_token_sha256=operation_token
    )
    assert tuple(record["phase"] for record in directory_records) == (
        "PENDING", "CREATED",
    )
    assert directory_records[-1]["operation_identity"] == (
        ForkedInstallerService._path_identity(response_parent)
    )
    assert sibling.read_bytes() == b"preserve-me"
    assert sibling.stat().st_ino == sibling_inode


@pytest.mark.parametrize("with_child", [False, True])
def test_response_operation_directory_requires_write_ahead_journal(
    with_child: bool, tmp_path,
) -> None:
    token = "d" * 64
    response_root = tmp_path / "protected-installer" / "response-artifacts"
    response_root.mkdir(mode=0o700, parents=True)
    orphan = response_root / token
    orphan.mkdir(mode=0o700)
    if with_child:
        (orphan / "attacker-created").write_bytes(b"not-owned")

    with pytest.raises(
        RuntimeError, match="INSTALLER_TEST_RESPONSE_DIRECTORY_ORPHAN"
    ):
        ForkedInstallerService._record_terminal_response(
            tmp_path,
            request_id="atomic-response",
    ...
    if with_child:
        assert (orphan / "attacker-created").read_bytes() == b"not-owned"
    directory_journal = (
        tmp_path / "protected-installer" / "response-directory-journals"
        / f"{token}.jsonl"
    )
    assert not directory_journal.exists()
...
def test_response_directory_replay_rejects_root_swap_before_mutation(
    boundary: str, tmp_path
) -> None:
    token = "d" * 64
    response_root = tmp_path / "protected-installer" / "response-artifacts"
    ...
    displaced = protected / "response-artifacts.displaced"
    journal = (
        protected / "response-directory-journals" / f"{token}.jsonl"
    )
    journal_before = journal.read_bytes()
    displaced_entries_before = tuple(
        sorted(path.name for path in response_root.iterdir())
    )
    os.rename(response_root, displaced)
    response_root.mkdir(mode=0o700)

    with pytest.raises(
        RuntimeError,
        match="INSTALLER_TEST_RESPONSE_DIRECTORY_ROOT_IDENTITY",
    ):
        ForkedInstallerService._ensure_response_operation_directory(
            tmp_path,
            operation_token_sha256=token,
    ...
    assert not tuple(response_root.iterdir())
    assert tuple(sorted(path.name for path in displaced.iterdir())) == (
        displaced_entries_before
    )
    assert journal.read_bytes() == journal_before


@pytest.mark.parametrize("boundary", RESPONSE_ARTIFACT_PUBLICATION_BOUNDARIES)
def test_live_service_response_publication_crash_recovers_to_complete(
    boundary: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    crashed = ForkedInstallerService.start(
        tmp_path / "service-crashed",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("INSTALLED",),
        publication_crash_after=boundary,
        manager_state_root=root,
    )
    first = InstallerOrchestrator.restart(root, crashed.socket_path)
    with pytest.raises(RuntimeError, match="INSTALLER_TRANSPORT_UNCERTAIN"):
        first.begin_install()
    first.close_after_uncertain_transport()
    crashed.wait(timeout=5.0)

    replacement = ForkedInstallerService.start(
        tmp_path / "service-replacement",
        construction_lock=construction,
        transaction_lock=transaction,
        script=(
            ("NO_JOURNAL_NO_MUTATION", "INSTALLED")
            if boundary.startswith("after_response_directory_")
            else ("RECOVERED",)
        ),
        manager_state_root=root,
    )
    restarted = InstallerOrchestrator.restart(root, replacement.socket_path)
    restarted.drive_to_terminal()
    snapshot = restarted.store.validated_snapshot()
    assert snapshot.terminal is True
    assert snapshot.cleanup_required is False
    operation_dir = (
        tmp_path / "protected-installer" / "response-artifacts"
        / INSTALL_REQUEST["payload"]["operation_token_sha256"]
    )
    assert not any(
        path.name.endswith(".pending") for path in operation_dir.iterdir()
    )
    assert all(path.name.endswith(".bin") for path in operation_dir.iterdir())
    replacement.stop()
...
def test_successful_install_finishes_independent_publication_validation(tmp_path) -> None:
    ...
    assert snapshot.authorization_sequence == 2
    ...
    assert records[0]["actor_previous_authorization_entry_sha256"] is None
    assert records[0]["actor_owner_transition"] == (
        INSTALL_AUTHORIZATION["owner_transition"]
    )
    install_response = _fixture_response_bytes(
        tmp_path, INSTALL_REQUEST
    )
    response_parent = (
        protected / "response-artifacts"
        / INSTALL_REQUEST["payload"]["operation_token_sha256"]
    )
    response_parent_fd = os.open(
        response_parent, os.O_RDONLY | os.O_DIRECTORY | os.O_CLOEXEC
    )
    try:
        assert read_protected_response_artifact(
            response_parent_fd,
            request_id=INSTALL_REQUEST["request_id"],
            expected_parent_identity=ForkedInstallerService._path_identity(
                response_parent
            ),
            expected_uid=os.getuid(),
            expected_gid=os.getgid(),
    ...
    owner = json.loads(manager.store.read_owner_record_bytes())
    contract = {
        "journal_payload": journal,
        "receipt_payload": receipt_bytes,
        "response_artifacts": {
            INSTALL_REQUEST["request_id"]: install_response,
        },
        "install_request": INSTALL_REQUEST,
        "install_authorization": INSTALL_AUTHORIZATION,
        "recovery_requests": (),
        ...
        "recovery_results": (),
        "owner_transition_history": INSTALL_OWNER_TRANSITIONS,
        "current_owner_record": owner,
    }
    assert validate_protected_publication_contract(**contract) == records

    for field, value in (
        ("owner_token_sha256", "9" * 64),
        ("staging_path", "/forged/staging"),
        ("cleanup_required", True),
        ("pending_local_transition", "VALIDATE_PUBLICATION"),
        ("phase", "BASE_RECEIPT_CREATED"),
        ("authorization_sequence", 1),
    ):
        forged_owner = json.loads(json.dumps(owner))
        forged_owner[field] = value
        with pytest.raises(
            RuntimeError,
            match=(
                "PROTECTED_RESPONSE_OWNER_IMMUTABLE_FIELD"
                "|PROTECTED_RESPONSE_OWNER_LIFECYCLE_FIELD"
                "|PROTECTED_RESPONSE_OWNER_TERMINAL_SEQUENCE"
            ),
        ):
            validate_protected_publication_contract(
                **{**contract, "current_owner_record": forged_owner}
            )

    forged_response = json.loads(install_response)
    forged_response["payload"]["component_hashes"]["base_python"] = "9" * 64
    forged_response_bytes = json.dumps(
        forged_response, sort_keys=True, separators=(",", ":")
    ).encode()
    forged_lines = journal[:-1].split(b"\n")
    forged_terminal = json.loads(forged_lines[-1])
    forged_terminal["terminal_response_sha256"] = hashlib.sha256(
        forged_response_bytes
    ).hexdigest()
    forged_lines[-1] = json.dumps(
        forged_terminal, sort_keys=True, separators=(",", ":")
    ).encode()
    with pytest.raises(
        RuntimeError, match="PROTECTED_RESPONSE_RECEIPT_BINDING"
    ):
        validate_protected_publication_contract(
            **{
                **contract,
                "journal_payload": b"\n".join(forged_lines) + b"\n",
                "response_artifacts": {
                    INSTALL_REQUEST["request_id"]: forged_response_bytes,
                },
            }
        )
    service.stop()
...
def test_recovery_appends_history_and_preserves_install_receipt(tmp_path) -> None:
    ...
    assert records[5]["actor_previous_authorization_entry_sha256"] == (
        RECOVERY_AUTHORIZATION["previous_entry_sha256"]
    )
    assert records[5]["actor_owner_transition"] == (
        recovery_authorization["owner_transition"]
    )
    recovery_response = _fixture_response_bytes(
        tmp_path, recovery_request
    )
    assert records[-1]["terminal_response_sha256"] == hashlib.sha256(
        recovery_response
    ).hexdigest()
    ...
    owner = json.loads(restarted.store.read_owner_record_bytes())
    response_artifacts = {
        INSTALL_REQUEST["request_id"]: install_response,
        recovery_request["request_id"]: recovery_response,
    }
    contract = {
        "journal_payload": complete_journal,
        ...
        "recovery_requests": (recovery_request,),
        "recovery_authorizations": (recovery_authorization,),
        "recovery_results": tuple(owner["recovery_results"]),
        "owner_transition_history": RECOVERY_OWNER_TRANSITIONS,
        "current_owner_record": owner,
    }
    assert validate_protected_publication_contract(**contract) == records

    tampered_results = json.loads(json.dumps(owner["recovery_results"]))
    tampered_results[-1]["terminal_response_sha256"] = "0" * 64
    result_body = {
        key: value
        for key, value in tampered_results[-1].items()
        if key != "result_sha256"
    }
    tampered_results[-1]["result_sha256"] = hashlib.sha256(
        json.dumps(result_body, sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
    tampered_owner = json.loads(json.dumps(owner))
    tampered_owner["recovery_results"] = tampered_results
    with pytest.raises(
        RuntimeError, match="PROTECTED_RESPONSE_RESULT_BINDING"
    ):
        validate_protected_publication_contract(
            **{
                **contract,
                "recovery_results": tuple(tampered_results),
                "current_owner_record": tampered_owner,
            }
        )
    service.stop()
    service.wait(timeout=5.0)
...
def test_initial_only_recovery_aborts_and_starts_fresh_attempt(tmp_path) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    transaction = tmp_path / "transaction.lock"
    DiskAuthorizationStore.bootstrap(
    ...
    restarted = InstallerOrchestrator.restart(root, recovery_service.socket_path)
    restarted.drive_to_terminal()
    requests = [json.loads(value) for value in recovery_service.request_bytes()]
    recovery_request, fresh_install_request = requests
    recovery_authorization = fixture_publication_authorization(
        recovery_request,
        previous=INSTALL_AUTHORIZATION,
        previous_owner_record=INSTALL_OWNER_RECORD,
    ...
    old_records = decode_exact_publication_journal(
        old_journal.read_bytes(),
        install_request=INSTALL_REQUEST,
        install_authorization=INSTALL_AUTHORIZATION,
        recovery_requests=(recovery_request,),
        recovery_authorizations=(recovery_authorization,),
    ...
    assert [record["phase"] for record in old_records] == [
        "INITIAL", "RECOVERY_STARTED", "STAGING_REMOVE_PENDING",
        "ABORTED_BEFORE_PREPARED",
    ]
    assert all(
    ...
    assert fresh_install_request["operation"] == "install_base_python"
    assert fresh_install_request["payload"]["construction_attempt"] == 2
    assert fresh_install_request["payload"]["operation_token_sha256"] != token
    assert fresh_install_request["payload"]["authorization_id"] != (
        INSTALL_REQUEST["payload"]["authorization_id"]
    )
    snapshot = restarted.store.validated_snapshot()
    assert snapshot.phase == "BASE_COMPLETE"
    assert snapshot.construction_attempt == 2
    recovery_service.stop()
    recovery_service.wait(timeout=5.0)
...
@pytest.mark.parametrize(
    "abort_boundary",
    (
        "RECOVERY_STARTED", "STAGING_REMOVE_PENDING", "STAGING_REMOVED",
        "after_exact_staging_rmdir", "after_staging_parent_fsync",
        "ABORTED_BEFORE_PREPARED",
    ),
)
def test_pre_prepared_abort_restarts_at_every_durable_boundary(
    abort_boundary: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    transaction = tmp_path / "transaction.lock"
    ...
    token = INSTALL_REQUEST["payload"]["operation_token_sha256"]
    published_parent = tmp_path / "protected-installer" / "published"
    staged = published_parent / f".staging-{token}"
    nested = staged / "partial" / "deep"
    nested.mkdir(parents=True)
    (nested / "payload.bin").write_bytes(b"partial-before-prepared")
    (staged / "partial-link").symlink_to("partial/deep/payload.bin")
    sibling = published_parent / "unrelated-sibling"
    sibling.mkdir()
    sibling_payload = sibling / "keep.bin"
    sibling_payload.write_bytes(b"preserve-me")
    sibling_identity = ForkedInstallerService._path_identity(sibling)

    crashing = ForkedInstallerService.start(
        tmp_path / "crashing-recovery",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("ABORTED_BEFORE_PREPARED",),
        publication_crash_after=abort_boundary,
        manager_state_root=root,
    )
    second = InstallerOrchestrator.restart(root, crashing.socket_path)
    with pytest.raises(Exception, match="UNCERTAIN|EOF"):
        second.drive_to_terminal()
    second.close_after_uncertain_transport()
    crashing.wait(timeout=5.0)
    recovery_bytes = crashing.request_bytes()[0]
    ...
    replay = ForkedInstallerService.start(
        tmp_path / "replay-recovery",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("ABORTED_BEFORE_PREPARED", "INSTALLED"),
        manager_state_root=root,
    )
    restarted = InstallerOrchestrator.restart(root, replay.socket_path)
    restarted.drive_to_terminal()
    assert replay.request_bytes()[0] == recovery_bytes
    fresh_request = json.loads(replay.request_bytes()[1])
    assert fresh_request["payload"]["construction_attempt"] == 2
    assert fresh_request["payload"]["operation_token_sha256"] != (
        INSTALL_REQUEST["payload"]["operation_token_sha256"]
    )
    journal = (
    ...
    assert old_records[-1]["phase"] == "ABORTED_BEFORE_PREPARED"
    assert all(record["final_tree_sha256"] is None for record in old_records)
    assert not staged.exists()
    assert sibling_payload.read_bytes() == b"preserve-me"
    assert ForkedInstallerService._path_identity(sibling) == sibling_identity
    replay.stop()
    replay.wait(timeout=5.0)


@pytest.mark.parametrize(
    "crash_boundary", tuple(FRESH_ATTEMPT_CRASH_BOUNDARIES)
)
def test_fresh_attempt_restarts_at_every_owner_durability_sub_boundary(
    ...
    expected = FRESH_ATTEMPT_CRASH_BOUNDARIES[crash_boundary]
    ...
    elif expected != FRESH_ATTEMPT_STATES["fresh_install_authorized"]:
...
@pytest.mark.parametrize(
    ...
    (
        "RECOVERY_STARTED", "RENAME_PENDING",
        "FINAL_RENAMED", "RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED",
        "RECOVERY_PUBLICATION_VALIDATED", "RECOVERY_RESPONSE_CREATED",
    ),
)
def test_same_recovery_generation_resumes_each_partial_recovery_prefix(
    ...
    install_service = ForkedInstallerService.start(
        ...
        transaction_lock=transaction,
        script=("INSTALLED",),
        publication_crash_after="PREPARED",
        manager_state_root=root,
    )
    first = InstallerOrchestrator.restart(root, install_service.socket_path)
    with pytest.raises(Exception, match="UNCERTAIN|EOF"):
    ...
    assert all(
        record["actor_authorization_sha256"]
        == recovery_authorization["authorization_sha256"]
        for record in completed_records[recovery_start:]
    )
    assert all(
        record["phase"] != "RECEIPT_CREATED"
        for record in completed_records[recovery_start:]
    )
    replay_service.stop()
...
def test_recovery_adopts_receipt_fsynced_before_journal_append(tmp_path) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    transaction = tmp_path / "transaction.lock"
    DiskAuthorizationStore.bootstrap(
    ...
    install_service = ForkedInstallerService.start(
        ...
        transaction_lock=transaction,
        script=("INSTALLED",),
        publication_crash_after="FINAL_RENAMED",
        manager_state_root=root,
    )
    first = InstallerOrchestrator.restart(root, install_service.socket_path)
    with pytest.raises(Exception, match="UNCERTAIN|EOF"):
    ...
    immutable_install_prefix = journal.read_bytes()
    ...
    crashing_recovery = ForkedInstallerService.start(
        ...
        transaction_lock=transaction,
        script=("RECOVERED",),
        publication_crash_after="RECEIPT_FSYNCED",
        manager_state_root=root,
    )
    second = InstallerOrchestrator.restart(
        root, crashing_recovery.socket_path
    ...
    protected = tmp_path / "protected-installer"
    receipt_paths = tuple((protected / "receipts").iterdir())
    ...
    assert prefix_records[-1]["phase"] == "RECOVERY_STARTED"
    ...
    restarted = InstallerOrchestrator.restart(root, replay.socket_path)
    restarted.drive_to_terminal()
    assert replay.request_bytes() == [recovery_bytes]
    assert receipt_path.read_bytes() == receipt_before
    assert receipt_path.stat().st_ino == receipt_inode_before
    completed_payload = journal.read_bytes()
    assert completed_payload.startswith(immutable_install_prefix)
    completed = decode_exact_publication_journal(
        completed_payload,
        install_request=INSTALL_REQUEST,
        install_authorization=INSTALL_AUTHORIZATION,
        recovery_requests=(recovery_request,),
    ...
    assert completed[-1]["phase"] == "RECOVERY_RESPONSE_CREATED"
    adoption = completed[-3]
    assert adoption["phase"] == (
        "RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED"
    )
    assert adoption["actor_request_id"] == recovery_request["request_id"]
    assert adoption["adopted_install_request_id"] == (
        INSTALL_REQUEST["request_id"]
    )
    assert adoption["adopted_install_response_sha256"] == hashlib.sha256(
        _fixture_response_bytes(tmp_path, INSTALL_REQUEST)
    ).hexdigest()
    assert not any(
        record["phase"] == "RECEIPT_CREATED"
        and record["sequence"] > len(
            immutable_install_prefix.rstrip(b"\n").split(b"\n")
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("parent_mount", "INSTALLER_TEST_RECEIPT_PARENT_IDENTITY"),
    ],
)
def test_receipt_adoption_rejects_each_identity_mutation(
    ...
    receipt = Path(publication["receipt_path"])
    terminal_response = ForkedInstallerService._encode_response(
        request=INSTALL_REQUEST,
        status="INSTALLED",
        publication=publication,
        fault=None,
    ...
    journal = Path(publication["journal_path"])
    journal_before = journal.read_bytes()
    complete = decode_exact_publication_journal(
        journal_before,
        install_request=INSTALL_REQUEST,
        install_authorization=INSTALL_AUTHORIZATION,
        owner_transition_history=INSTALL_OWNER_TRANSITIONS,
    )
    assert complete[-1]["phase"] == "RECEIPT_CREATED"
    receipt_inode = receipt.stat().st_ino
    if mutation in {"uid", "gid", "parent_mount"}:
        config = tmp_path / "protected-installer" / "installer-config.json"
        value = json.loads(config.read_bytes())
        if mutation == "uid":
            value["input_uid"] += 1
        elif mutation == "gid":
            ...
            value["receipt_parent_identity"]["mount_id"] += 1
        config.chmod(0o644)
        config.write_text(
            json.dumps(value, sort_keys=True, separators=(",", ":")),
            encoding="ascii",
        ...
        with config.open("rb") as stream:
            os.fsync(stream.fileno())
        config.chmod(0o444)
        ForkedInstallerService._fsync_dir(config.parent)
    elif mutation == "mode":
        receipt.chmod(0o644)
    elif mutation == "type":
        receipt.unlink()
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "actor_previous_authorization_entry_sha256", "actor_owner_transition",
...
def test_each_journal_authorization_binding_rejects_one_field_mutation(
    ...
    if field == "actor_authorization_sequence":
        record[field] += 1
    elif field == "actor_previous_authorization_entry_sha256":
        record[field] = "9" * 64
...
def test_authorization_codec_rejects_each_provenance_mutation(
    ...
    if field != "authorization_sha256":
        _rehash_fixture_authorization(authorization, INSTALL_REQUEST)
    publication = {
        "source_parent_identity": {
            "device": 1, "inode": 2, "mount_id": 3,
            "mode": 0o700, "uid": os.getuid(), "gid": os.getgid(),
...
def test_authorization_rejects_valid_lowercase_wrong_digest() -> None:
    forged = json.loads(json.dumps(INSTALL_AUTHORIZATION))
    forged["authorization_sha256"] = "9" * 64
    publication = {
        "source_parent_identity": {
            "device": 1, "inode": 2, "mount_id": 3,
            "mode": 0o700, "uid": os.getuid(), "gid": os.getgid(),
    ...
    with pytest.raises(RuntimeError, match="PUBLICATION_AUTHORIZATION_DIGEST"):
        append_publication_phase(
            b"",
            install_request=INSTALL_REQUEST,
            install_authorization=forged,
            recovery_requests=(),
            recovery_authorizations=(),
            actor_request=INSTALL_REQUEST,
            actor_authorization=forged,
            phase="INITIAL",
            publication=publication,
            observed_root_mode=0o700,
            owner_transition_history=INSTALL_OWNER_TRANSITIONS,
...
def _fixture_two_generation_owner() -> tuple[
    ...
    reactivated = fixture_owner_record_for_reactivation(
        INSTALL_AUTHORIZATION,
        reactivation,
        previous_owner_record=RECOVERY_OWNER_RECORD,
    )
    request = {
        **RECOVERY_REQUEST,
        "request_id": "recover-base-python-generation-2",
        "payload": {
            **RECOVERY_REQUEST["payload"],
            "authorization_id": "recovery-authorization-2",
            "recovery_generation": 2,
            "recovery_token_sha256": "8" * 64,
        },
    }
    authorization = fixture_publication_authorization(
        ...
        previous_owner_record=reactivated,
    )
    owner = fixture_owner_record_for_authorization(
        authorization,
        request=request,
        previous_owner_record=reactivated,
    )
    return request, authorization, owner


def test_install_and_recovery_authorizations_use_distinct_full_schemas() -> None:
    assert set(INSTALL_AUTHORIZATION) == INSTALL_AUTHORIZATION_KEYS
    assert set(RECOVERY_AUTHORIZATION) == RECOVERY_AUTHORIZATION_KEYS
    assert INSTALL_AUTHORIZATION["kind"] == "install"
    assert RECOVERY_AUTHORIZATION["kind"] == "recovery"
    assert INSTALL_AUTHORIZATION["operation"] == "install_base_python"
...
@pytest.mark.parametrize(
    "mutation",
    (
        "gapped_generation", "reordered_authorizations", "cross_kind",
        "broken_authorization_link", "gapped_result_generation",
        "broken_result_link", "broken_result_digest",
        "missing_result_prefix", "reordered_results",
    ),
)
def test_owner_history_rejects_gap_order_kind_and_hash_breaks(
    mutation: str,
) -> None:
    _, generation_2_authorization, owner_bytes = _fixture_two_generation_owner()
    if mutation in {"missing_result_prefix", "reordered_results"}:
        transition = fixture_install_reactivation_transition(
            INSTALL_AUTHORIZATION,
            previous_authorization=generation_2_authorization,
            previous_transition=generation_2_authorization["owner_transition"],
            previous_owner_record=owner_bytes,
        )
        owner_bytes = fixture_owner_record_for_reactivation(
            INSTALL_AUTHORIZATION,
            transition,
            previous_owner_record=owner_bytes,
        )
    owner = json.loads(owner_bytes)
    if mutation == "gapped_generation":
        owner["recovery_authorizations"][1]["recovery_generation"] = 3
    elif mutation == "reordered_authorizations":
        owner["recovery_authorizations"] = list(
            reversed(owner["recovery_authorizations"])
        )
    elif mutation == "cross_kind":
        owner["recovery_authorizations"][0] = json.loads(
            json.dumps(INSTALL_AUTHORIZATION)
        )
    elif mutation == "broken_authorization_link":
        owner["recovery_authorizations"][1]["previous_entry_sha256"] = "9" * 64
    elif mutation == "missing_result_prefix":
        owner["recovery_results"] = owner["recovery_results"][1:]
    elif mutation == "reordered_results":
        owner["recovery_results"] = list(reversed(owner["recovery_results"]))
    else:
        result = owner["recovery_results"][0]
        if mutation == "gapped_result_generation":
            result["recovery_generation"] = 2
        elif mutation == "broken_result_link":
            result["previous_result_sha256"] = "9" * 64
        else:
            result["result_sha256"] = "9" * 64
        if mutation != "broken_result_digest":
            body = {
                key: result[key]
                for key in result if key != "result_sha256"
            }
    ...
    if mutation in {"gapped_generation", "broken_authorization_link"}:
        authorization = owner["recovery_authorizations"][1]
        authorization["authorization_sha256"] = hashlib.sha256(
            json.dumps(
                {
                    key: value
    ...
    with pytest.raises(RuntimeError, match="PUBLICATION_(AUTHORIZATION|OWNER)"):
        encode_authorization_owner_record(owner)


def test_recovery_authorization_rejects_valid_digest_with_wrong_request() -> None:
    forged = json.loads(json.dumps(RECOVERY_AUTHORIZATION))
    forged["request_sha256"] = "9" * 64
    forged["authorization_sha256"] = hashlib.sha256(
        json.dumps(
            {
                key: value
    ...
    with pytest.raises(
        RuntimeError, match="PUBLICATION_AUTHORIZATION_REQUEST_BINDING"
    ):
        append_publication_phase(
            b"",
            install_request=INSTALL_REQUEST,
            install_authorization=INSTALL_AUTHORIZATION,
            recovery_requests=(RECOVERY_REQUEST,),
            recovery_authorizations=(forged,),
            actor_request=INSTALL_REQUEST,
            actor_authorization=INSTALL_AUTHORIZATION,
            phase="INITIAL",
            publication={
...
def test_store_authorization_binds_actual_full_owner_record(tmp_path) -> None:
    root = tmp_path / "manager-state"
    DiskAuthorizationStore.bootstrap(
        root,
        install_request=INSTALL_BYTES,
        construction_lock=tmp_path / "construction.lock",
        transaction_lock=tmp_path / "transaction.lock",
    )
    ...
    after = store.read_owner_record_bytes()
    assert after == encode_authorization_owner_record(
        transition_entry.to_owner_record
    )
    authorization = snapshot.install_authorization
    transition = authorization["owner_transition"]
    persisted_owner = json.loads(after)
    assert persisted_owner["install_authorization"] == authorization
    assert persisted_owner["active_service_authorization"] == {
        "kind": "install", "generation": 0,
    }
    assert persisted_owner["active_authorization_sha256"] == (
        authorization["authorization_sha256"]
    )
    assert "to_owner_record_sha256" not in transition
    assert authorization["authorization_sha256"].encode() in after
...
@pytest.mark.parametrize(
    "mutation",
    (
        "construction_identity", "run_id", "owner_token", "operation_token",
        "owner_process", "state", "phase", "staging_path",
        "construction_attempt", "attempt_status", "cleanup_required",
        "pending_action", "pending_local_transition", "heartbeat",
        "last_error", "install_authorization", "recovery_authorizations",
        "recovery_results", "active_reference",
        "active_authorization_sha256", "authorization_sequence",
        "transport_uncertain", "response_uncertain",
        "last_preacquisition_status", "locked_response_records",
        "publication_mirrors", "protected_validation",
        "component_hashes", "final_tree_sha256", "receipt_sha256",
        "terminal_response_status",
    ),
)
def test_store_owner_transition_rejects_each_security_field_mutation(
    ...
    changed = json.loads(store.read_owner_record_bytes())
    if mutation == "construction_identity":
        changed["base_python_intake_sha256"] = "9" * 64
    elif mutation == "run_id":
        changed["run_id"] = "forged-run"
    elif mutation == "construction_attempt":
        changed["construction_attempt"] += 1
    elif mutation == "owner_token":
        changed["owner_token_sha256"] = "8" * 64
    elif mutation == "operation_token":
        changed["operation_token_sha256"] = "7" * 64
    elif mutation == "owner_process":
        changed["owner_start_ticks"] += 1
    elif mutation == "state":
        changed["state"] = "COMPLETE"
    elif mutation == "phase":
        changed["phase"] = "BASE_ALLOCATED"
    elif mutation == "staging_path":
        changed["staging_path"] = "/forged/staging"
    elif mutation == "attempt_status":
        changed["attempt_status"] = "complete"
    elif mutation == "cleanup_required":
        changed["cleanup_required"] = False
    elif mutation == "pending_action":
        changed["pending_action"] = "forged-action"
    elif mutation == "pending_local_transition":
        changed["pending_local_transition"] = "FORGED_TRANSITION"
    elif mutation == "heartbeat":
        changed["heartbeat_monotonic_ns"] += 1
    elif mutation == "last_error":
        changed["last_error"] = "forged-error"
    elif mutation == "install_authorization":
        changed["install_authorization"] = {
            **snapshot.install_authorization,
            "authorization_id": "forged-install",
        }
    elif mutation == "recovery_authorizations":
        changed["recovery_authorizations"] = [snapshot.install_authorization]
    elif mutation == "recovery_results":
        changed["recovery_results"] = [{"forged": True}]
    elif mutation == "active_reference":
        changed["active_service_authorization"]["generation"] = 99
    elif mutation == "active_authorization_sha256":
        changed["active_authorization_sha256"] = "6" * 64
    elif mutation == "authorization_sequence":
        changed["authorization_sequence"] += 1
    elif mutation == "transport_uncertain":
        changed["transport_uncertain"] = True
    elif mutation == "response_uncertain":
        changed["response_uncertain"] = True
    elif mutation == "last_preacquisition_status":
        changed["last_preacquisition_status"] = "INSTALLER_TRANSACTION_BUSY"
    elif mutation == "locked_response_records":
        changed["locked_response_records"] = [{"forged": True}]
    elif mutation == "publication_mirrors":
        changed["publication_mirrors"] = ["BASE_FINAL_RENAMED"]
    elif mutation == "protected_validation":
        changed["protected_journal_validated"] = True
    elif mutation == "component_hashes":
        changed["component_hashes"] = {"base_python": "5" * 64}
    elif mutation == "final_tree_sha256":
        changed["final_tree_sha256"] = "4" * 64
    elif mutation == "receipt_sha256":
        changed["receipt_sha256"] = "3" * 64
    elif mutation == "terminal_response_status":
        changed["terminal_response_status"] = "INSTALLED"
    else:
        raise AssertionError(mutation)
    changed_bytes = json.dumps(
        changed, sort_keys=True, separators=(",", ":")
    ).encode()
    with pytest.raises(RuntimeError, match="PUBLICATION_OWNER"):
        validate_owner_transition_against_records(
            snapshot.install_authorization["owner_transition"],
            from_owner_record=before,
            to_owner_record=changed_bytes,
            expected_authorization=snapshot.install_authorization,
            creates_authorization=True,
        )
...
@pytest.mark.parametrize(
    "mutation",
    ("owner_token", "staging_path", "phase", "cleanup", "history"),
)
def test_coherently_rehashed_transition_rejects_forbidden_owner_delta(
    mutation: str,
) -> None:
    before = INSTALL_BEFORE_OWNER_RECORD
    changed = json.loads(INSTALL_OWNER_RECORD)
    pending = encode_pending_publication_authorization(
        request=INSTALL_REQUEST,
        authorization_sequence=1,
        previous_authorization=None,
    )
    changed["install_authorization"] = pending
    changed["active_authorization_sha256"] = None
    if mutation == "owner_token":
        changed["owner_token_sha256"] = "8" * 64
    elif mutation == "staging_path":
        changed["staging_path"] = "/forged/staging"
    elif mutation == "phase":
        ...
        changed["locked_response_records"] = [{"forged": True}]
    projection = encode_authorization_owner_projection(
        changed, creates_authorization=True
    )
    transition = encode_owner_transition(
        authorization_id=INSTALL_REQUEST["payload"]["authorization_id"],
        authorization_sequence=1,
        from_authorization_id=None,
        from_authorization_sequence=0,
        from_owner_record_sha256=hashlib.sha256(before).hexdigest(),
        from_pending_action="prepare_install_authorization",
        previous_transition=None,
        reason="activate_authorization",
        to_owner_record_projection_sha256=hashlib.sha256(projection).hexdigest(),
        to_pending_action="install_base_python",
    )
    forged_authorization = encode_publication_authorization(
        request=INSTALL_REQUEST,
        authorization_sequence=1,
        previous_authorization=None,
        owner_transition=transition,
    )
    changed["install_authorization"] = forged_authorization
    changed["active_authorization_sha256"] = forged_authorization[
        "authorization_sha256"
    ]
    after = encode_authorization_owner_record(changed)
    with pytest.raises(
        RuntimeError, match="PUBLICATION_OWNER_TRANSITION_FORBIDDEN_DELTA"
    ):
        encode_owner_transition_entry(
            transition,
            from_owner_record=before,
            to_owner_record=after,
            expected_authorization=forged_authorization,
            creates_authorization=True,
        )


def test_privileged_journal_rejects_transition_only_history() -> None:
    publication = {
        "source_parent_identity": {
            "device": 1, "inode": 2, "mount_id": 3,
            "mode": 0o700, "uid": os.getuid(), "gid": os.getgid(),
...
def test_owner_transition_history_allows_heartbeat_gap() -> None:
...
def test_owner_transition_history_rejects_busy_diagnostic_owner_gap() -> None:
    with pytest.raises(
        ...
        _initial_journal_after_owner_gap(
            {
                "heartbeat_monotonic_ns": 2,
                "heartbeat_wall_ns": 2,
                "last_preacquisition_status": "INSTALLER_TRANSACTION_BUSY",
            }
        )


def test_owner_transition_history_allows_same_kind_forward_phase_gap() -> None:
    journal = _initial_journal_after_owner_gap(
        {
            "phase": "BASE_METADATA_NORMALIZED",
            "heartbeat_monotonic_ns": 2,
            "heartbeat_wall_ns": 2,
        }
    )
...
def test_owner_transition_history_rejects_cross_kind_phase_gap() -> None:
    with pytest.raises(
        RuntimeError, match="PUBLICATION_OWNER_TRANSITION_PHASE_KIND"
    ):
        _initial_journal_after_owner_gap(
            {
...
def test_owner_transition_history_rejects_unowned_lifecycle_gap(
    field: str, value: object
) -> None:
    with pytest.raises(
        RuntimeError, match="PUBLICATION_OWNER_TRANSITION_RECORD_CHAIN"
    ):
        _initial_journal_after_owner_gap({field: value})


def test_service_history_accepts_only_final_heartbeat_tail() -> None:
    current = json.loads(INSTALL_OWNER_RECORD)
    current["heartbeat_monotonic_ns"] = 2
    current["heartbeat_wall_ns"] = 2
    transitions = validate_owner_history_for_service(
        INSTALL_OWNER_TRANSITIONS,
        current_owner_record=current,
    )
    assert transitions[-1] == INSTALL_TRANSITION_ENTRY["transition"]


def test_service_history_rejects_same_kind_phase_in_live_tail() -> None:
    current = json.loads(INSTALL_OWNER_RECORD)
    current["phase"] = "BASE_METADATA_NORMALIZED"
    with pytest.raises(RuntimeError, match="PUBLICATION_OWNER_SERVICE_TAIL"):
        validate_owner_history_for_service(
            INSTALL_OWNER_TRANSITIONS,
            current_owner_record=current,
        )
...
def test_service_history_rejects_unowned_cleanup_in_final_tail() -> None:
    current = json.loads(INSTALL_OWNER_RECORD)
    current["cleanup_required"] = False
    with pytest.raises(RuntimeError, match="PUBLICATION_OWNER_SERVICE_TAIL"):
        validate_owner_history_for_service(
            INSTALL_OWNER_TRANSITIONS,
            current_owner_record=current,
...
def test_no_journal_generation_and_reactivation_remain_in_full_history() -> None:
    install_owner_record = INSTALL_OWNER_RECORD
    generation_1_owner_record = RECOVERY_OWNER_RECORD
    reactivation = fixture_install_reactivation_transition(
        INSTALL_AUTHORIZATION,
        previous_authorization=RECOVERY_AUTHORIZATION,
        previous_transition=RECOVERY_AUTHORIZATION["owner_transition"],
        previous_owner_record=generation_1_owner_record,
    )
    ...
    generation_2_authorization = fixture_publication_authorization(
        generation_2_request,
        previous=RECOVERY_AUTHORIZATION,
        previous_transition=reactivation,
    ...
    generation_2_owner_record = fixture_owner_record_for_authorization(
        ...
        request=generation_2_request,
        previous_owner_record=reactivated_owner_record,
    )
    authorization_history = (
        RECOVERY_AUTHORIZATION, generation_2_authorization,
    )
    request_history = (RECOVERY_REQUEST, generation_2_request)
    transition_history = (
        INSTALL_TRANSITION_ENTRY,
        RECOVERY_TRANSITION_ENTRY,
        fixture_reactivation_transition_entry(
            ...
            reactivation,
            previous_owner_record=generation_1_owner_record,
            target_owner_record=reactivated_owner_record,
        ),
        fixture_owner_transition_entry(
            generation_2_authorization,
            previous_owner_record=reactivated_owner_record,
            target_owner_record=generation_2_owner_record,
            creates_authorization=True,
        ),
    )
    assert [
        entry["transition"]["to_authorization_sequence"]
    ...
    validate_owner_transition_against_records(
        ...
        expected_authorization=RECOVERY_AUTHORIZATION,
        creates_authorization=True,
    )
    validate_owner_transition_against_records(
        reactivation,
    ...
    validate_owner_transition_against_records(
        ...
        expected_authorization=generation_2_authorization,
        creates_authorization=True,
    )
    publication = {
        "source_parent_identity": {
        ...
        "component_hashes": {"base_python": "2" * 64},
        "final_tree_sha256": "3" * 64,
        "root_identity": {
            "device": 1, "inode": 4, "mount_id": 3,
            "mode": 0o555, "uid": os.getuid(), "gid": os.getgid(),
        },
    }
    journal = append_publication_phase(
        b"",
        install_request=INSTALL_REQUEST,
        install_authorization=INSTALL_AUTHORIZATION,
        recovery_requests=request_history,
        recovery_authorizations=authorization_history,
        actor_request=INSTALL_REQUEST,
        actor_authorization=INSTALL_AUTHORIZATION,
        phase="INITIAL",
        publication=publication,
    ...
    journal = append_publication_phase(
        ...
        recovery_requests=request_history,
        recovery_authorizations=authorization_history,
        actor_request=generation_2_request,
        actor_authorization=generation_2_authorization,
        phase="RECOVERY_STARTED",
        publication=publication,
        observed_root_mode=0o555,
        owner_transition_history=transition_history,
    ...
    assert records[-1]["actor_request_id"] == generation_2_request["request_id"]
    for incomplete_history in (
        transition_history[:1] + transition_history[2:],
        transition_history[:2] + transition_history[3:],
    ):
        with pytest.raises(RuntimeError, match="TRANSITION|AUTHORIZATION"):
            decode_exact_publication_journal(
                journal,
                install_request=INSTALL_REQUEST,
                install_authorization=INSTALL_AUTHORIZATION,
                recovery_requests=request_history,
                recovery_authorizations=authorization_history,
                owner_transition_history=incomplete_history,
            )
    with pytest.raises(RuntimeError, match="PUBLICATION_AUTHORIZATION_PREVIOUS"):
        decode_exact_publication_journal(
            journal,
            install_request=INSTALL_REQUEST,
            install_authorization=INSTALL_AUTHORIZATION,
            recovery_requests=(generation_2_request,),
            recovery_authorizations=(generation_2_authorization,),
            owner_transition_history=transition_history,
        )


@pytest.mark.parametrize("boundary", RECEIVE_TIME_ARTIFACT_BOUNDARIES)
def test_receive_time_artifact_crash_replays_byte_identical_request(
    boundary: str,
    tmp_path,
) -> None:
    root = tmp_path / "manager-state"
    ...
    if pid == 0:
        manager = InstallerOrchestrator.restart(root, service.socket_path)
        manager.begin_install(
            fault_hook=lambda name: os._exit(85) if name == boundary else None
        )
        os._exit(0)
    _, status = os.waitpid(pid, 0)
    assert os.waitstatus_to_exitcode(status) == 85

    # The response artifact is service-durable, but the manager has not yet
    # committed its result/owner transition.  Restart therefore replays the
    # exact request and the service adopts the same immutable artifact.
    restarted = InstallerOrchestrator.restart(root, service.socket_path)
    restarted.drive_to_terminal()
    assert service.request_bytes() == [INSTALL_BYTES, INSTALL_BYTES]
...
def test_no_journal_artifact_before_result_commit_is_adopted_on_replay(
    tmp_path,
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    if pid == 0:
        ...
        manager.drive_to_terminal(
            fault_hook=lambda name: os._exit(83)
            if name == "before_result_append" else None
        )
        os._exit(0)
    _, status = os.waitpid(pid, 0)
    assert os.waitstatus_to_exitcode(status) == 83
    requests_before = service.request_bytes()
    recovery_before = next(
        body for body in requests_before
        if json.loads(body)["operation"] == "recover_base_python"
    )
    recovery_id = json.loads(recovery_before)["request_id"]
    artifact_path = (
        tmp_path / "protected-installer" / "response-artifacts"
        / json.loads(recovery_before)["payload"]["operation_token_sha256"]
        / f"{hashlib.sha256(recovery_id.encode()).hexdigest()}.bin"
    )
    artifact_bytes = artifact_path.read_bytes()
    artifact_inode = artifact_path.stat().st_ino

    restarted = InstallerOrchestrator.restart(root, service.socket_path)
    restarted.drive_to_terminal()
    recovery_replays = [
        body for body in service.request_bytes()
        if json.loads(body).get("request_id") == recovery_id
    ]
    assert recovery_replays == [recovery_before, recovery_before]
    assert artifact_path.read_bytes() == artifact_bytes
    assert artifact_path.stat().st_ino == artifact_inode
    assert restarted.store.validated_snapshot().terminal is True
    service.stop()
    service.wait(timeout=5.0)
...
def test_manager_calls_shared_contract_before_first_publication_mirror(
    ...
    def reject_before_mirror(**_kwargs) -> None:
        snapshot = manager.store.validated_snapshot()
        observed.append(snapshot.publication_mirrors)
        raise RuntimeError("TEST_SHARED_CONTRACT_REJECTED")

    monkeypatch.setattr(
        manager.store,
        "validate_publication_before_mirror",
        reject_before_mirror,
    )
    with pytest.raises(RuntimeError, match="TEST_SHARED_CONTRACT_REJECTED"):
        manager.begin_install()
    snapshot = manager.store.validated_snapshot()
    assert observed == [()]
    assert snapshot.phase == "BASE_METADATA_NORMALIZED"
    assert snapshot.publication_mirrors == ()
    assert snapshot.cleanup_required is True
    manager.ownership._close_construction_lock()
    service.stop()
    service.wait(timeout=5.0)


def test_extra_operation_scoped_response_artifact_blocks_first_mirror(
    tmp_path,
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    def inject_extra(name: str) -> None:
        if name != "after_handoff_close":
            return
        extra = response_operation / ("9" * 64 + ".bin")
        fd = os.open(
            extra,
            os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_CLOEXEC,
            0o400,
        ...
        try:
            os.write(fd, b"extra durable response\n")
            os.fsync(fd)
        finally:
            os.close(fd)
        ForkedInstallerService._fsync_dir(response_operation)

    manager = InstallerOrchestrator.restart(root, service.socket_path)
    with pytest.raises(
        RuntimeError, match="PROTECTED_RESPONSE_ARTIFACT_NAMESPACE"
    ):
        manager.begin_install(fault_hook=inject_extra)
    snapshot = manager.store.validated_snapshot()
    assert snapshot.publication_mirrors == ()
    assert snapshot.cleanup_required is True
    manager.ownership._close_construction_lock()
...
def test_replayed_install_success_uses_same_durable_handoff_path(tmp_path) -> None:
    ...
    assert snapshot.terminal_response_status == "INSTALLED"
    requests = service.request_bytes()
    assert [body for body in requests if body == INSTALL_BYTES] == [
        INSTALL_BYTES,
        INSTALL_BYTES,
    ]
    recovery_request = next(
        json.loads(body) for body in requests
        if json.loads(body)["operation"] == "recover_base_python"
    )
    no_journal_response = _fixture_response_bytes(
        tmp_path, recovery_request
    )
    decoded = decode_exact_response_payload(
        no_journal_response, request_id=recovery_request["request_id"]
    )
    assert decoded.status == "NO_JOURNAL_NO_MUTATION"
    matching_result = next(
        result for result in snapshot.recovery_results
        if result.recovery_generation
        == recovery_request["payload"]["recovery_generation"]
    )
    assert matching_result.terminal_response_sha256 == hashlib.sha256(
        no_journal_response
    ).hexdigest()
    # Terminal install journal + consumed no-journal result jointly determine
    # the exact artifact set; the production terminalization above would fail
    # if either evidence source were omitted or treated as an extra file.
    expected_basenames = {
        f"{hashlib.sha256(request_id.encode()).hexdigest()}.bin"
        for request_id in (
            INSTALL_REQUEST["request_id"], recovery_request["request_id"],
        )
    }
    response_parent = (
        tmp_path / "protected-installer" / "response-artifacts"
    ...
    assert {path.name for path in response_parent.glob("*.bin")} == (
        expected_basenames
    )
    service.stop()
    service.wait(timeout=5.0)
...
def test_no_journal_reactivation_is_durable_before_handoff_release(
    tmp_path,
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    def inspect(name: str) -> None:
        snapshot = restarted.store.validated_snapshot()
        if (
            name == "before_handoff_close"
            and len(snapshot.recovery_results) == 1
            and snapshot.terminal_response_status is None
        ):
            assert snapshot.active_authorization_kind == "install"
            assert snapshot.pending_local_transition is None
            assert snapshot.next_wire_request_bytes == INSTALL_BYTES
            assert snapshot.authorization_sequence == 3
            with pytest.raises(BlockingIOError):
                fcntl.flock(contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
            observations.append("reactivated-and-fsynced-while-locked")
        elif name == "after_handoff_close" and observations == [
            "reactivated-and-fsynced-while-locked"
        ]:
            fcntl.flock(contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
            fcntl.flock(contender, fcntl.LOCK_UN)
            observations.append("released-after-atomic-reactivation")

    restarted.drive_to_terminal(fault_hook=inspect)
    assert observations[:2] == [
        "reactivated-and-fsynced-while-locked",
        "released-after-atomic-reactivation",
    ]
    os.close(contender)
    service.stop()
    service.wait(timeout=5.0)
...
def test_locked_malformed_install_response_is_durable_before_unlock(
    ...
    transition_history = manager.store.read_owner_transition_history()
    assert transition_history[-1]["transition"]["reason"] == (
        "activate_recovery_after_locked_install_response"
    )
    assert transition_history[-1]["to_owner_record"][
        "locked_response_records"
    ] == list(snapshot.locked_response_records)
...
@pytest.mark.parametrize("response_fault", ["partial_body", "malformed_schema"])
def test_malformed_recovery_appends_audit_without_new_generation(
    response_fault: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    dropped = ForkedInstallerService.start(
        tmp_path / "service-dropped",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("TRANSPORT_DROPPED_AFTER_JOURNAL",),
        manager_state_root=root,
    )
    first = InstallerOrchestrator.restart(root, dropped.socket_path)
    with pytest.raises(RuntimeError, match="INSTALLER_TRANSPORT_UNCERTAIN"):
        first.begin_install()
    first.close_after_uncertain_transport()
    dropped.wait(timeout=5.0)

    malformed = ForkedInstallerService.start(
        tmp_path / "service-malformed-recovery",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("RECOVERED",),
        response_fault=response_fault,
    ...
    restarted = InstallerOrchestrator.restart(root, malformed.socket_path)
    with pytest.raises(RuntimeError, match="INSTALLER_RESPONSE_UNCERTAIN"):
        restarted.drive_to_terminal()
    snapshot = restarted.store.validated_snapshot()
    assert snapshot.active_authorization_kind == "recovery"
    assert snapshot.active_recovery_generation == 1
    assert len(snapshot.recovery_authorizations) == 1
    assert len(snapshot.locked_response_records) == 1
    history = restarted.store.read_owner_transition_history()
    audit = history[-1]
    assert audit["transition"]["reason"] == (
        "append_locked_recovery_response_audit"
    )
    assert audit["transition"]["from_authorization_sequence"] == (
        audit["transition"]["to_authorization_sequence"]
    )
    assert audit["from_owner_record"]["active_service_authorization"] == (
        audit["to_owner_record"]["active_service_authorization"]
    )
    restarted.close_after_uncertain_transport()
    malformed.stop()
    malformed.wait(timeout=5.0)


@pytest.mark.parametrize("response_fault", ["partial_body", "malformed_schema"])
def test_locked_response_history_survives_recovery_and_complete(
    response_fault: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    first = InstallerOrchestrator.restart(root, malformed.socket_path)
    with pytest.raises(RuntimeError, match="INSTALLER_RESPONSE_UNCERTAIN"):
        first.begin_install()
    uncertain = first.store.validated_snapshot()
    locked_prefix = uncertain.locked_response_records
    assert len(locked_prefix) == 1
    first.close_after_uncertain_transport()
    malformed.stop()
    malformed.wait(timeout=5.0)

    replacement = ForkedInstallerService.start(
        tmp_path / "service-recovery",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("RECOVERED",),
        manager_state_root=root,
    ...
    terminal = restarted.store.validated_snapshot()
    assert terminal.terminal is True
    assert terminal.locked_response_records[: len(locked_prefix)] == locked_prefix
    validate_locked_response_history(
        terminal.locked_response_records,
        authorization_by_request_sha256={
            authorization["request_sha256"]: (
                authorization["authorization_sha256"],
                authorization.get("recovery_generation"),
            )
            for authorization in (
                terminal.install_authorization,
                *terminal.recovery_authorizations,
            )
        },
        required_prefix=locked_prefix,
    )
    replacement.stop()
    replacement.wait(timeout=5.0)
...
def test_zero_fd_transaction_busy_preserves_install_for_byte_identical_retry(
    ...
    manager = InstallerOrchestrator.restart(root, service.socket_path)
    owner_before = manager.store.read_owner_record_bytes()
    ...
    assert snapshot.last_preacquisition_status == "INSTALLER_TRANSACTION_BUSY"
    assert manager.store.read_owner_record_bytes() == owner_before
    diagnostics = (
        root / "preacquisition-diagnostics.jsonl"
    ).read_text(encoding="utf-8").splitlines()
    assert len(diagnostics) == 1
    assert json.loads(diagnostics[0])["status"] == (
        "INSTALLER_TRANSACTION_BUSY"
    )
    ...
    terminal = restarted.store.validated_snapshot()
    assert terminal.terminal is True
    # Clearing the install active reference is itself an authorization
    # transition, so terminalization consumes the next sequence value.
    assert terminal.authorization_sequence == 2
...
def test_zero_fd_response_requires_exact_preacquisition_codec(
    ...
    assert snapshot.active_authorization_kind == "recovery"
    assert snapshot.active_recovery_generation == 1
    assert snapshot.owner_transition_history[-1].transition["reason"] == (
        "activate_recovery_after_transport_uncertain_install"
    )
    manager.close_after_uncertain_transport()
    fcntl.flock(transaction_holder, fcntl.LOCK_UN)
    os.close(transaction_holder)
    ...
    replacement = ForkedInstallerService.start(
        ...
        construction_lock=construction,
        transaction_lock=transaction,
        script=("NO_JOURNAL_NO_MUTATION", "INSTALLED"),
        manager_state_root=root,
    )
    restarted = InstallerOrchestrator.restart(root, replacement.socket_path)
    restarted.drive_to_terminal()
...
def test_invalid_handoff_uses_stable_lock_uncertainty_only(
    ...
    assert snapshot.active_authorization_kind == "recovery"
    assert snapshot.active_recovery_generation == 1
    history = manager.store.read_owner_transition_history()
    assert history[-1]["transition"]["reason"] == (
        "activate_recovery_after_transport_uncertain_install"
    )
    manager.close_after_uncertain_transport()
    service.stop()
...
@pytest.mark.parametrize("response_fault", ["missing_handoff", "wrong_handoff"])
def test_invalid_recovery_handoff_replays_same_generation_from_real_store(
    response_fault: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    second = InstallerOrchestrator.restart(root, malformed.socket_path)
    with pytest.raises(OSError, match="stop-after-transport-transition"):
        second.drive_to_terminal(
            fault_hook=lambda name: (_ for _ in ()).throw(
                OSError("stop-after-transport-transition")
            ) if name == "after_transport_uncertain_transition" else None,
        )
    snapshot = second.store.validated_snapshot()
    assert snapshot.active_authorization_kind == "recovery"
    assert snapshot.active_recovery_generation == 1
    assert snapshot.next_wire_request_bytes == snapshot.last_wire_request_bytes
    history = second.store.read_owner_transition_history()
    assert history[-1]["transition"]["reason"] == (
        "record_transport_uncertain_recovery_replay"
    )
    assert history[-1]["transition"]["from_authorization_sequence"] == (
        history[-1]["transition"]["to_authorization_sequence"]
    )
    recovery_bytes = malformed.request_bytes()[0]
    second.close_after_uncertain_transport()
    malformed.stop()
    malformed.wait(timeout=5.0)

    replacement = ForkedInstallerService.start(
        tmp_path / "replacement-recovery",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("RECOVERED",),
        manager_state_root=root,
    ...
    restarted = InstallerOrchestrator.restart(root, replacement.socket_path)
    restarted.drive_to_terminal()
    assert replacement.request_bytes() == [recovery_bytes]
    assert restarted.store.validated_snapshot().terminal is True
    replacement.stop()
    replacement.wait(timeout=5.0)
...
def test_restart_locks_construction_before_transaction_reconciliation(
    monkeypatch, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    service = ForkedInstallerService.start(
        ...
        transaction_lock=transaction,
        script=("INSTALLED",),
        response_fault="missing_handoff",
        manager_state_root=root,
    )
    pid = os.fork()
    if pid == 0:
        manager = InstallerOrchestrator.restart(root, service.socket_path)
        manager.begin_install(
            fault_hook=lambda name: os._exit(86)
            if name == "after_owner_transition_prepared_parent_fsync"
            else None,
        )
        os._exit(0)
    _, status = os.waitpid(pid, 0)
    ...
    original = DiskAuthorizationStore.reconcile_owner_transition_transaction
    observed: list[str] = []

    def assert_construction_locked(store, *args, **kwargs) -> None:
        contender = os.open(
            construction, os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW
        )
        try:
            with pytest.raises(BlockingIOError):
                fcntl.flock(contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
            observed.append("locked-before-reconcile")
        finally:
            os.close(contender)
    ...
    monkeypatch.setattr(
        DiskAuthorizationStore,
        "reconcile_owner_transition_transaction",
        assert_construction_locked,
    )
    ownership = InstallerManagerOwnership.restart(root)
    assert observed == ["locked-before-reconcile"]
    assert ownership.store.validated_snapshot().transport_uncertain is True
    ownership.close_without_handoff()


@pytest.mark.parametrize("boundary", OWNER_TRANSITION_TRANSACTION_BOUNDARIES)
def test_owner_transition_transaction_recovers_every_write_ahead_boundary(
    boundary: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    failing = ForkedInstallerService.start(
        tmp_path / "failing-service",
        construction_lock=construction,
        transaction_lock=transaction,
        script=("INSTALLED",),
        response_fault="missing_handoff",
    ...
    pid = os.fork()
    if pid == 0:
        manager = InstallerOrchestrator.restart(root, failing.socket_path)
        manager.begin_install(
            fault_hook=lambda name: os._exit(86) if name == boundary else None,
        )
        os._exit(0)
    ...
    assert os.waitstatus_to_exitcode(status) == 86
    failing.stop()
    failing.wait(timeout=5.0)

    protected_before_replay = _snapshot_protected_installer_tree(tmp_path)
    reconciliation_owner = InstallerManagerOwnership.restart(root)
    store = reconciliation_owner.store
    committed, pending = decode_owner_transition_transaction_journal(
        store.read_owner_transition_transaction_bytes()
    )
    assert pending is None
    assert tuple(store.read_owner_transition_history_unreconciled()) == (
        committed
    )
    owner = json.loads(store.read_owner_record_bytes())
    assert owner == committed[-1]["to_owner_record"]
    active = owner["active_service_authorization"]
    next_status = (
        "RECOVERED"
        if active is not None and active["kind"] == "recovery"
        else "INSTALLED"
    )
    reconciliation_owner.close_without_handoff()

    replacement = ForkedInstallerService.start(
        tmp_path / "replacement-service",
        construction_lock=construction,
        transaction_lock=transaction,
        script=(next_status,),
        manager_state_root=root,
    ...
    restarted.drive_to_terminal()
    assert restarted.store.validated_snapshot().terminal is True
    if next_status == "INSTALLED":
        assert replacement.request_bytes() == [INSTALL_BYTES]
        assert _snapshot_protected_installer_tree(tmp_path) == (
            protected_before_replay
        )
    replacement.stop()
    replacement.wait(timeout=5.0)
...
@pytest.mark.parametrize("boundary", OWNER_TRANSITION_TRANSACTION_BOUNDARIES)
def test_recovery_replay_transition_recovers_every_write_ahead_boundary(
    boundary: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    owner = json.loads(store.read_owner_record_bytes())
    assert owner == committed[-1]["to_owner_record"]
    assert owner["active_service_authorization"] == {
        "kind": "recovery", "generation": 1,
    }
    reconciliation_owner.close_without_handoff()
...
@pytest.mark.parametrize("boundary", OWNER_TRANSITION_TRANSACTION_BOUNDARIES)
def test_locked_install_uncertainty_holds_both_locks_through_reconciliation(
    boundary: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
    ...
    manager = InstallerOrchestrator.restart(root, malformed.socket_path)
    with pytest.raises(Exception, match="HANDOFF_PERSISTENCE_UNCERTAIN"):
        manager.begin_install(
            fault_hook=lambda name: (_ for _ in ()).throw(OSError(name))
            if name == boundary else None,
        )
    transaction_contender = os.open(
        transaction, os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW
    )
    construction_contender = os.open(
        construction, os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW
    ...
    with pytest.raises(BlockingIOError):
        fcntl.flock(transaction_contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
    with pytest.raises(BlockingIOError):
        fcntl.flock(construction_contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
    manager.reconcile_locked_handoff()
    fcntl.flock(transaction_contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
    ...
    manager.close_after_uncertain_transport()
    fcntl.flock(construction_contender, fcntl.LOCK_EX | fcntl.LOCK_NB)
    os.close(transaction_contender)
    os.close(construction_contender)
    malformed.stop()
    malformed.wait(timeout=5.0)
...
@pytest.mark.parametrize("boundary", OWNER_TRANSITION_TRANSACTION_BOUNDARIES)
def test_locked_recovery_audit_holds_both_locks_through_reconciliation(
    boundary: str, tmp_path
) -> None:
    root = tmp_path / "manager-state"
    construction = tmp_path / "construction.lock"
...
Its default state machine drops the first install response, returns the same `NO_JOURNAL_NO_MUTATION` result for every replay of recovery generation 1 until the manager persists it, retains generation 1 in the immutable authorization history, appends the independent install-reactivation owner transition, creates/fsyncs the canonical install publication journal and receipt and drops the byte-identical second install response, then creates generation 2 whose authorization digest links to generation 1 while its activation transition links to the intervening reactivation transition. The owner authorization sequence is strictly monotonic across all four transitions—install 1, recovery generation 1, install reactivation, and recovery generation 2—so the sentinel requires `[1, 2, 3, 4]`; reactivation never reuses the install authorization's creation-time sequence. It appends only generation 2 as a publication actor and returns the exact `RECOVERED` response whose digest was fsynced first. The service and manager pass the complete authorization, result, owner-transition, response-artifact, and current-owner evidence to the shared protected-publication validator; histories may contain no-journal authorizations and transitions that never appear as journal actors, but omitting or changing any such entry breaks its independent canonical chain. It atomically publishes exact response bytes through fully fsynced temporary inodes into request-ID-hashed, mode-`0444` protected fixture artifacts before journal terminal records or sends, including `NO_JOURNAL_NO_MUTATION`. Parameterized scripts additionally exercise immediate `INSTALLED`, zero-FD `INSTALLER_TRANSACTION_BUSY` followed by byte-identical retry, `INITIAL`-only abort followed by a genuinely fresh attempt/token, and `NO_JOURNAL_NO_MUTATION` followed by byte-identical replay ending in `INSTALLED`. A phase-crash mode calls `_exit(87)` at every response-directory pending/mkdir/parent-fsync/created boundary, every temporary-create/write/fsync/chmod/publish/parent-fsync boundary, immediately after each per-record fsync, and after receipt or response publication but before its binding journal/result append. Every replacement service is given only the manager-state root. After acquiring `transaction.lock`, it reopens the actual `DiskAuthorizationStore` request, complete authorization/result histories, nested owner-transition entries, and current owner record, validates the active request/digest and heartbeat-only tail, and derives the next action from those bytes. The fixture API has no synthetic resume-owner parameters. A broken response socket never advances authorization state; request generation plus the fsynced service journal/result and response artifact determine the response. Test-only shutdown signals the owned child only after the manager verifies terminal state; there is no unauthenticated stop request. `InstallerOrchestrator` uses the production `sendmsg` session sender and `recv_installer_reply`; no test calls a persist method with a prebuilt reply. Receive-time artifact-validation crashes occur before manager result/owner durability and therefore require an actual byte-identical wire replay; post-handoff publication-validation crashes occur after that durability boundary and require no replay. A fresh orchestrator reopens only the fsynced prefix and cannot mark terminal until the response-directory journal, protected publication journal, receipt, exact union of terminal/result/adoption-selected response artifacts, recovery-result chain, full current owner, and published root have been jointly validated and every mirror plus `COMPLETE` has been separately fsynced. Assertions prove the install prefix remains a byte-for-byte prefix after recovery; existing receipt and response-artifact bytes, SHA-256, and inode never change; missing receipts and response artifacts are reconstructed only from install-bound fields; actual install replays are byte-identical; all generations are inside `payload`; every accepted request carried the stable construction-lock FD; and zero-FD busy replies create no recovery generation. `RecoveryHarness` remains only a pure hash-chain unit test. Native tests in Step 6 cover every service-side phase barrier. None calls systemd or requires root.
...
def test_installer_config_contains_all_stable_lock_identities() -> None:
    ...
    expected = {
        ...
        "transaction_lock_mode", "protected_response_root",
        "response_parent_device", "response_parent_inode",
        "response_parent_mount_id", "response_parent_uid",
        "response_parent_gid", "response_parent_mode",
        "mode_policy_version", "request_limit_bytes",
    }
...
OWNER_RECORD_KEYS = {
    "schema_version", "construction_kind", "closure_id",
    "descriptor_sha256", "base_python_intake_sha256", "run_id",
    "owner_token_sha256", "operation_token_sha256", "host_name",
    "boot_id", "owner_pid", "owner_start_ticks", "state", "phase",
    "staging_path", "final_path", "construction_attempt",
    "attempt_status", "cleanup_required", "pending_action",
    "pending_local_transition", "heartbeat_monotonic_ns",
    "heartbeat_wall_ns", "last_error", "install_authorization",
    "recovery_authorizations", "recovery_results",
    "active_service_authorization", "active_authorization_sha256",
    "authorization_sequence", "transport_uncertain",
    "response_uncertain", "last_preacquisition_status",
    "locked_response_records", "publication_mirrors",
    "protected_journal_validated", "protected_receipt_validated",
    "published_root_validated", "component_hashes",
    "final_tree_sha256", "receipt_sha256", "terminal_response_status",
}
ACTIVE_SERVICE_AUTHORIZATION_KEYS = {
    "kind", "generation",
}
INSTALL_AUTHORIZATION_KEYS = {
    "kind", "authorization_id", "authorization_sequence",
    "authorization_sha256", "operation", "request_sha256",
    "profile_sha256", "construction_kind", "source_id", "destination_id",
    "descriptor_or_intake_sha256", "construction_attempt",
    "expected_source_kind", "expected_destination_kind",
    "owner_token_sha256", "operation_token_sha256", "owner_transition",
}
RECOVERY_AUTHORIZATION_KEYS = INSTALL_AUTHORIZATION_KEYS | {
    "recovery_generation", "recovery_token_sha256", "reason",
    "previous_entry_sha256",
}
RECOVERY_RESULT_KEYS = {
    "recovery_generation", "authorization_sha256", "request_sha256",
    "terminal_response_sha256", "terminal_phase",
    "transaction_lock_identity_sha256", "previous_result_sha256",
    "result_sha256",
}
LOCKED_RESPONSE_RECORD_KEYS = {
    "schema_version", "sequence", "decision_request_sha256",
    "authorization_sha256", "recovery_generation",
    "received_prefix_sha256", "error_code",
    "transaction_lock_identity_sha256", "previous_entry_sha256",
    "record_sha256",
}
RESPONSE_DIRECTORY_JOURNAL_ENTRY_KEYS = {
    "schema_version", "sequence", "phase", "operation_token_sha256",
    "operation_basename", "response_root_identity", "operation_identity",
    "previous_entry_sha256",
}
OWNER_TRANSITION_ENTRY_KEYS = {
    "transition", "from_owner_record", "to_owner_record",
}
OWNER_TRANSITION_TRANSACTION_FRAME_KEYS = {
    "schema_version", "sequence", "phase", "transaction_id",
    "prepared_entry", "before_owner_sha256", "after_owner_sha256",
    "prepared_frame_sha256", "previous_frame_sha256",
}
OWNER_HEARTBEAT_KEYS = {
    "heartbeat_monotonic_ns", "heartbeat_wall_ns",
}
OWNER_PHASE_GAP_KEYS = {*OWNER_HEARTBEAT_KEYS, "phase"}
OWNER_PHASES_BY_KIND = {
    "base_python": (
        "BASE_ALLOCATED", "BASE_COPIED", "BASE_METADATA_NORMALIZED",
        "BASE_FINAL_RENAMED", "BASE_RECEIPT_CREATED", "BASE_COMPLETE",
    ),
...
PUBLICATION_JOURNAL_ENTRY_KEYS = PUBLICATION_RECEIPT_KEYS | {
    ...
    "response_operation_identity", "observed_root_mode",
    ...
    "actor_previous_authorization_entry_sha256", "actor_owner_transition",
    "actor_owner_transition_sha256",
    "adopted_install_request_id", "adopted_install_response_sha256",
}
INSTALL_PUBLICATION_PHASES = (
    "INITIAL", "PREPARED", "RENAME_PENDING", "FINAL_RENAMED",
    "RECEIPT_CREATED",
)
PRE_PREPARED_ABORT_PHASES = (
    "STAGING_REMOVE_PENDING", "ABORTED_BEFORE_PREPARED",
)
RECOVERY_PUBLICATION_PHASES = (
...
def decode_exact_response_directory_journal(
    payload: bytes,
    *,
    operation_token_sha256: str,
    ...
    _require_sha256(
        operation_token_sha256, code="RESPONSE_DIRECTORY_OPERATION_TOKEN"
    )
    if payload == b"" and allow_empty:
        return ()
    if not payload.endswith(b"\n"):
        raise RuntimeError("RESPONSE_DIRECTORY_JOURNAL_TERMINATOR")
    encoded_records = payload[:-1].split(b"\n")
    if len(encoded_records) not in {1, 2}:
        raise RuntimeError("RESPONSE_DIRECTORY_JOURNAL_LENGTH")
    ...
    for sequence, encoded in enumerate(encoded_records, 1):
        ...
        if (
            ...
            or set(record) != RESPONSE_DIRECTORY_JOURNAL_ENTRY_KEYS
            or _canonical_json(record) != encoded
        ):
            raise RuntimeError("RESPONSE_DIRECTORY_JOURNAL_SCHEMA")
        expected_phase = "PENDING" if sequence == 1 else "CREATED"
        if (
            record["schema_version"] != 1
            or record["sequence"] != sequence
            or record["phase"] != expected_phase
            or record["operation_token_sha256"] != operation_token_sha256
            or record["operation_basename"] != operation_token_sha256
            or record["previous_entry_sha256"] != previous
        ):
            raise RuntimeError("RESPONSE_DIRECTORY_JOURNAL_CHAIN")
        _validate_publication_identity(
            record["response_root_identity"],
            code="RESPONSE_DIRECTORY_ROOT_IDENTITY",
        )
        if root_identity is None:
            root_identity = record["response_root_identity"]
        elif record["response_root_identity"] != root_identity:
            raise RuntimeError("RESPONSE_DIRECTORY_ROOT_IDENTITY")
        if sequence == 1:
            if record["operation_identity"] is not None:
                raise RuntimeError("RESPONSE_DIRECTORY_PENDING_IDENTITY")
        else:
            _validate_publication_identity(
                record["operation_identity"],
                code="RESPONSE_DIRECTORY_OPERATION_IDENTITY",
            )
...
def append_response_directory_phase(
    existing: bytes,
    *,
    operation_token_sha256: str,
    phase: str,
    response_root_identity: Mapping[str, object],
    operation_identity: Mapping[str, object] | None,
) -> bytes:
    records = decode_exact_response_directory_journal(
        existing,
        operation_token_sha256=operation_token_sha256,
    ...
    expected = "PENDING" if not records else "CREATED"
    if phase != expected or len(records) >= 2:
        raise RuntimeError("RESPONSE_DIRECTORY_JOURNAL_NEXT_PHASE")
    record = {
        "schema_version": 1,
        "sequence": len(records) + 1,
        "phase": phase,
        "operation_token_sha256": operation_token_sha256,
        "operation_basename": operation_token_sha256,
        "response_root_identity": dict(response_root_identity),
        "operation_identity": (
            None if operation_identity is None else dict(operation_identity)
        ),
        "previous_entry_sha256": (
            None
            if not records
            else hashlib.sha256(existing[:-1].split(b"\n")[-1]).hexdigest()
        ),
    }
    candidate = existing + _canonical_json(record) + b"\n"
    decode_exact_response_directory_journal(
        candidate, operation_token_sha256=operation_token_sha256
    )
    return candidate
...
def encode_authorization_owner_record(
    ...
    encoded = _canonical_json(owner_record)
    _decode_authorization_owner_record(encoded)
    return encoded


def _owner_authorizations(
    value: Mapping[str, object],
) -> list[Mapping[str, object]]:
    install = value["install_authorization"]
    recovery = value["recovery_authorizations"]
    authorizations = [] if install is None else [install]
    return [*authorizations, *recovery]


def _authorization_entry_sha256(
    authorization: Mapping[str, object],
) -> str:
    return hashlib.sha256(_canonical_json(authorization)).hexdigest()


def _result_digest_document(result: Mapping[str, object]) -> dict[str, object]:
    return {
        key: result[key]
        for key in RECOVERY_RESULT_KEYS - {"result_sha256"}
    }


def _locked_response_digest_document(
    record: Mapping[str, object],
) -> dict[str, object]:
    return {
        key: record[key]
        for key in LOCKED_RESPONSE_RECORD_KEYS - {"record_sha256"}
    }


def encode_locked_response_record(
    *,
    ...
    body = {
        "schema_version": 1,
        "sequence": 1 if previous_record is None else (
            previous_record["sequence"] + 1
        ),
        "decision_request_sha256": decision_request_sha256,
        "authorization_sha256": authorization_sha256,
        "recovery_generation": recovery_generation,
        "received_prefix_sha256": received_prefix_sha256,
        "error_code": error_code,
        "transaction_lock_identity_sha256": (
            transaction_lock_identity_sha256
        ),
        "previous_entry_sha256": (
            None if previous_record is None else previous_record["record_sha256"]
        ),
    }
    return {
        **body,
        "record_sha256": hashlib.sha256(_canonical_json(body)).hexdigest(),
    }


def validate_locked_response_history(
    records: Sequence[Mapping[str, object]],
    *,
    authorization_by_request_sha256: Mapping[str, tuple[str, int | None]],
    required_prefix: Sequence[Mapping[str, object]] = (),
) -> tuple[dict[str, object], ...]:
    """Validate the append-only malformed/partial-response audit chain."""
    if len(records) < len(required_prefix) or list(
        records[: len(required_prefix)]
    ) != list(required_prefix):
        raise RuntimeError("LOCKED_RESPONSE_HISTORY_PREFIX")
    validated: list[dict[str, object]] = []
    for sequence, raw in enumerate(records, start=1):
        if type(raw) is not dict or set(raw) != LOCKED_RESPONSE_RECORD_KEYS:
            raise RuntimeError("LOCKED_RESPONSE_RECORD_SCHEMA")
        record = dict(raw)
        if record["schema_version"] != 1 or record["sequence"] != sequence:
            raise RuntimeError("LOCKED_RESPONSE_RECORD_SEQUENCE")
        binding = authorization_by_request_sha256.get(
            record["decision_request_sha256"]
        )
        if binding is None:
            raise RuntimeError("LOCKED_RESPONSE_RECORD_REQUEST")
        if (record["authorization_sha256"], record["recovery_generation"]) != (
            binding
        ):
            raise RuntimeError("LOCKED_RESPONSE_RECORD_AUTHORIZATION")
        for key in (
            "decision_request_sha256", "authorization_sha256",
            "received_prefix_sha256", "transaction_lock_identity_sha256",
            "record_sha256",
        ):
            _require_sha256(record[key], code="LOCKED_RESPONSE_RECORD_DIGEST")
        generation = record["recovery_generation"]
        if generation is not None and (
            type(generation) is not int or generation <= 0
        ):
            raise RuntimeError("LOCKED_RESPONSE_RECORD_GENERATION")
        if type(record["error_code"]) is not str or not record["error_code"]:
            raise RuntimeError("LOCKED_RESPONSE_RECORD_ERROR")
        previous = None if not validated else validated[-1]["record_sha256"]
        if record["previous_entry_sha256"] != previous:
            raise RuntimeError("LOCKED_RESPONSE_RECORD_CHAIN")
        expected = hashlib.sha256(
            _canonical_json(_locked_response_digest_document(record))
        ).hexdigest()
        if record["record_sha256"] != expected:
            raise RuntimeError("LOCKED_RESPONSE_RECORD_DIGEST")
        validated.append(record)
    return tuple(validated)


def encode_recovery_result(
    *,
    ...
    body = {
        "recovery_generation": recovery_authorization["recovery_generation"],
        "authorization_sha256": recovery_authorization["authorization_sha256"],
        "request_sha256": recovery_authorization["request_sha256"],
        "terminal_response_sha256": terminal_response_sha256,
        "terminal_phase": terminal_phase,
        "transaction_lock_identity_sha256": transaction_lock_identity_sha256,
    ...
    return {
        **body,
        "result_sha256": hashlib.sha256(_canonical_json(body)).hexdigest(),
    }


def _active_owner_authorization(
    value: Mapping[str, object],
) -> Mapping[str, object] | None:
    active = value["active_service_authorization"]
    if active is None:
        return None
    if active["kind"] == "install":
        matches = [value["install_authorization"]]
    else:
        generation = active["generation"]
        recoveries = value["recovery_authorizations"]
        matches = (
            [recoveries[generation - 1]]
            if 0 < generation <= len(recoveries)
            else []
        )
    if len(matches) != 1:
        raise RuntimeError("PUBLICATION_OWNER_ACTIVE_AUTHORIZATION_REFERENCE")
    authorization = matches[0]
    if authorization is None or (
        active["kind"] == "recovery"
        and authorization["authorization_sequence"]
...
def _validate_owner_record_mapping(
    value: object, *, allow_pending_authorization: bool = False
) -> dict[str, object]:
    if type(value) is not dict or set(value) != OWNER_RECORD_KEYS:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_SCHEMA")
    if value["schema_version"] != 1:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_SCHEMA")
    for key in ("owner_token_sha256", "operation_token_sha256"):
        _require_sha256(value[key], code="PUBLICATION_OWNER_RECORD_TOKEN")
    for key in ("descriptor_sha256", "base_python_intake_sha256"):
        if value[key] is not None:
            _require_sha256(value[key], code="PUBLICATION_OWNER_RECORD_IDENTITY")
    kind = value["construction_kind"]
    if kind == "base_python":
        if (
            value["closure_id"] is not None
            or value["descriptor_sha256"] is not None
            or type(value["base_python_intake_sha256"]) is not str
            or type(value["construction_attempt"]) is not int
            or value["construction_attempt"] <= 0
            or value["attempt_status"] not in {
                "active", "aborted-before-prepared", "complete",
            }
        ):
            raise RuntimeError("PUBLICATION_OWNER_RECORD_CONSTRUCTION")
    elif kind == "closure":
        if (
            type(value["closure_id"]) is not str
            or not value["closure_id"]
            or type(value["descriptor_sha256"]) is not str
            or value["base_python_intake_sha256"] is not None
            or value["construction_attempt"] is not None
            or value["attempt_status"] is not None
        ):
            raise RuntimeError("PUBLICATION_OWNER_RECORD_CONSTRUCTION")
    else:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_CONSTRUCTION")
    for key in ("run_id", "host_name", "boot_id", "state", "phase"):
        if type(value[key]) is not str or not value[key]:
            raise RuntimeError("PUBLICATION_OWNER_RECORD_VALUE")
    for key in ("owner_pid", "owner_start_ticks"):
        if type(value[key]) is not int or value[key] <= 0:
            raise RuntimeError("PUBLICATION_OWNER_RECORD_VALUE")
    for key in ("heartbeat_monotonic_ns", "heartbeat_wall_ns"):
        if type(value[key]) is not int or value[key] < 0:
            raise RuntimeError("PUBLICATION_OWNER_RECORD_VALUE")
    for key in ("staging_path", "final_path", "pending_action"):
        if value[key] is not None and (
            type(value[key]) is not str or not value[key]
        ):
            raise RuntimeError("PUBLICATION_OWNER_RECORD_VALUE")
    if value["pending_local_transition"] is not None and (
        type(value["pending_local_transition"]) is not str
        or not value["pending_local_transition"]
    ):
        raise RuntimeError("PUBLICATION_OWNER_RECORD_VALUE")
    for key in (
        "cleanup_required", "transport_uncertain", "response_uncertain",
        "protected_journal_validated", "protected_receipt_validated",
        "published_root_validated",
    ):
    ...
    for key in (
        ...
        if type(value[key]) is not list:
            raise RuntimeError("PUBLICATION_OWNER_RECORD_HISTORY")
    locked_authorizations = {
        authorization["request_sha256"]: (
            authorization["authorization_sha256"],
            authorization.get("recovery_generation"),
        )
        for authorization in _owner_authorizations(value)
        if authorization is not None
        and authorization.get("request_sha256") is not None
    }
    validate_locked_response_history(
        value["locked_response_records"],
        authorization_by_request_sha256=locked_authorizations,
    )
    if any(type(entry) is not str or not entry for entry in value["publication_mirrors"]):
        raise RuntimeError("PUBLICATION_OWNER_RECORD_HISTORY")
    if type(value["component_hashes"]) is not dict:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_COMPONENTS")
    if any(type(name) is not str or not name for name in value["component_hashes"]):
        raise RuntimeError("PUBLICATION_OWNER_RECORD_COMPONENTS")
    for digest in value["component_hashes"].values():
        _require_sha256(digest, code="PUBLICATION_OWNER_RECORD_COMPONENTS")
    for key in ("final_tree_sha256", "receipt_sha256"):
        if value[key] is not None:
            _require_sha256(value[key], code="PUBLICATION_OWNER_RECORD_DIGEST")
    if type(value["authorization_sequence"]) is not int or (
        value["authorization_sequence"] < 0
    ):
        raise RuntimeError("PUBLICATION_OWNER_RECORD_AUTHORIZATION_SEQUENCE")
    _validate_owner_authorization_histories(
        value, allow_pending_authorization=allow_pending_authorization
    )
    active = value["active_service_authorization"]
    if active is not None:
        if type(active) is not dict or set(active) != (
            ...
            raise RuntimeError("PUBLICATION_OWNER_ACTIVE_REFERENCE")
        if (
            active["kind"] not in {"install", "recovery"}
            or type(active["generation"]) is not int
            or active["generation"] < 0
        ):
            raise RuntimeError("PUBLICATION_OWNER_ACTIVE_REFERENCE")
        if (active["kind"] == "install") != (active["generation"] == 0):
            raise RuntimeError("PUBLICATION_OWNER_ACTIVE_REFERENCE")
    active_authorization = _active_owner_authorization(value)
    active_digest = value["active_authorization_sha256"]
    if active_authorization is None:
        if active_digest is not None:
            raise RuntimeError("PUBLICATION_OWNER_ACTIVE_AUTHORIZATION_BINDING")
    elif allow_pending_authorization:
        if (
            active_digest is not None
            or active_authorization["authorization_sha256"] is not None
            or active_authorization["owner_transition"] is not None
        ):
            raise RuntimeError("PUBLICATION_OWNER_PENDING_AUTHORIZATION")
    else:
        _require_sha256(
            active_digest,
            code="PUBLICATION_OWNER_ACTIVE_AUTHORIZATION_BINDING",
        )
        if active_authorization["authorization_sha256"] != active_digest:
            raise RuntimeError("PUBLICATION_OWNER_ACTIVE_AUTHORIZATION_BINDING")
    if value["install_authorization"] is None:
        if (
            value["recovery_authorizations"]
            or value["recovery_results"]
            or active is not None
            or value["authorization_sequence"] != 0
        ):
            raise RuntimeError("PUBLICATION_OWNER_AUTHORIZATION_BOOTSTRAP")
    elif value["authorization_sequence"] <= 0:
        raise RuntimeError("PUBLICATION_OWNER_AUTHORIZATION_SEQUENCE")
    for key in ("last_error", "last_preacquisition_status", "terminal_response_status"):
        if value[key] is not None and (
            type(value[key]) is not str or not value[key]
        ):
            raise RuntimeError("PUBLICATION_OWNER_RECORD_VALUE")
...
def encode_authorization_owner_projection(
    owner_record: Mapping[str, object], *, creates_authorization: bool
) -> bytes:
    value = json.loads(_canonical_json(owner_record))
    _validate_owner_record_mapping(
        value, allow_pending_authorization=creates_authorization
    )
...
def _decode_authorization_owner_record(payload: bytes) -> dict[str, object]:
    ...
    if _canonical_json(value) != payload:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_NONCANONICAL")
    return _validate_owner_record_mapping(value)
...
def validate_owner_transition_against_records(
    ...
    *,
    ...
    expected_authorization: Mapping[str, object],
    creates_authorization: bool,
) -> None:
    ...
    active_authorization = _active_owner_authorization(after)
    if (
        active_authorization != expected_authorization
        or after["active_authorization_sha256"]
        != expected_authorization["authorization_sha256"]
    ):
        raise RuntimeError("PUBLICATION_OWNER_ACTIVE_AUTHORIZATION_BINDING")
    projected = json.loads(_canonical_json(after))
    if creates_authorization:
        projected_active = _active_owner_authorization(projected)
        if projected_active is None:
            raise RuntimeError("PUBLICATION_OWNER_PENDING_AUTHORIZATION")
        projected_active["authorization_sha256"] = None
        projected_active["owner_transition"] = None
        projected["active_authorization_sha256"] = None
    projection = encode_authorization_owner_projection(
        projected, creates_authorization=creates_authorization
    )
    if hashlib.sha256(projection).hexdigest() != transition[
        "to_owner_record_projection_sha256"
    ]:
        raise RuntimeError("PUBLICATION_OWNER_TRANSITION_TO_PROJECTION")
    expected_before = (
        None
        if _active_owner_authorization(before) is None
        else _active_owner_authorization(before)["authorization_id"],
        before["authorization_sequence"],
        before["pending_action"],
    )
    expected_after = (
        _active_owner_authorization(after)["authorization_id"],
    ...
    changed = {
        key for key in OWNER_RECORD_KEYS if before[key] != after[key]
    }
    if transition["reason"] == "activate_authorization":
        allowed = {
            "install_authorization", "recovery_authorizations",
            "active_service_authorization", "active_authorization_sha256",
            "authorization_sequence", "pending_action", "state",
            "transport_uncertain", "response_uncertain", "last_error",
        }
        if changed - allowed:
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_FORBIDDEN_DELTA")
        if expected_authorization["kind"] == "install":
            if (
                before["install_authorization"] is not None
                or after["install_authorization"] != expected_authorization
                or after["recovery_authorizations"]
                != before["recovery_authorizations"]
                or after["recovery_results"] != before["recovery_results"]
                or after["active_service_authorization"]
                != {"kind": "install", "generation": 0}
                or before["state"] != "CONSTRUCTING"
                or after["state"] != "INSTALLING"
            ):
                raise RuntimeError("PUBLICATION_OWNER_TRANSITION_INSTALL_DELTA")
        else:
            if (
                after["install_authorization"]
                != before["install_authorization"]
                or after["recovery_authorizations"]
                != [*before["recovery_authorizations"], expected_authorization]
                or after["recovery_results"] != before["recovery_results"]
                or after["active_service_authorization"]
                != {
                    "kind": "recovery",
                    "generation": expected_authorization[
                        "recovery_generation"
                    ],
                }
                or after["state"] != before["state"]
            ):
                raise RuntimeError("PUBLICATION_OWNER_TRANSITION_RECOVERY_DELTA")
        if (
            after["transport_uncertain"] is not False
            or after["response_uncertain"] is not False
            or after["last_error"] is not None
        ):
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_ACTIVATION_STATUS")
    elif transition["reason"] == (
        "activate_recovery_after_locked_install_response"
    ):
        allowed = {
            "recovery_authorizations", "active_service_authorization",
            "active_authorization_sha256", "authorization_sequence",
            "pending_action", "transport_uncertain", "response_uncertain",
            "last_error", "locked_response_records",
        }
        before_active = _active_owner_authorization(before)
        if changed - allowed:
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_FORBIDDEN_DELTA")
        if (
            before_active is None
            or before_active["kind"] != "install"
            or expected_authorization["kind"] != "recovery"
            or expected_authorization["recovery_generation"]
            != len(before["recovery_authorizations"]) + 1
            or after["install_authorization"]
            != before["install_authorization"]
            or after["recovery_authorizations"]
            != [*before["recovery_authorizations"], expected_authorization]
            or after["recovery_results"] != before["recovery_results"]
            or after["locked_response_records"][:-1]
            != before["locked_response_records"]
            or len(after["locked_response_records"])
            != len(before["locked_response_records"]) + 1
            or after["active_service_authorization"]
            != {
                "kind": "recovery",
                "generation": expected_authorization["recovery_generation"],
            }
            or after["authorization_sequence"]
            != before["authorization_sequence"] + 1
            or after["response_uncertain"] is not True
            or after["transport_uncertain"] is not False
            or after["last_error"]
            != after["locked_response_records"][-1]["error_code"]
        ):
            raise RuntimeError("PUBLICATION_OWNER_LOCKED_INSTALL_RECOVERY_DELTA")
    elif transition["reason"] == (
        "activate_recovery_after_transport_uncertain_install"
    ):
        allowed = {
        ...
        if (
            ...
            != [*before["recovery_authorizations"], expected_authorization]
            or after["recovery_results"] != before["recovery_results"]
            or after["locked_response_records"]
            != before["locked_response_records"]
            or after["active_service_authorization"]
            != {
                "kind": "recovery",
            ...
            or after["authorization_sequence"]
            != before["authorization_sequence"] + 1
            or after["transport_uncertain"] is not True
            or after["response_uncertain"] is not False
            or type(after["last_error"]) is not str
            or not 0 < len(after["last_error"]) <= 128
        ):
            raise RuntimeError(
                "PUBLICATION_OWNER_TRANSPORT_INSTALL_RECOVERY_DELTA"
            )
    elif transition["reason"] == "append_locked_recovery_response_audit":
        allowed = {
            "response_uncertain", "last_error", "locked_response_records",
        }
        before_active = _active_owner_authorization(before)
        if changed - allowed:
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_FORBIDDEN_DELTA")
        if (
            before_active is None
            or before_active["kind"] != "recovery"
            or expected_authorization != before_active
            or after["install_authorization"]
            != before["install_authorization"]
            or after["recovery_authorizations"]
            != before["recovery_authorizations"]
            ...
            != len(before["locked_response_records"]) + 1
            or after["active_service_authorization"]
            != before["active_service_authorization"]
            or after["active_authorization_sha256"]
            != before["active_authorization_sha256"]
            or after["authorization_sequence"]
            != before["authorization_sequence"]
            or after["pending_action"] != before["pending_action"]
            or after["response_uncertain"] is not True
            or after["transport_uncertain"]
            != before["transport_uncertain"]
            or after["last_error"]
            != after["locked_response_records"][-1]["error_code"]
        ):
            raise RuntimeError("PUBLICATION_OWNER_LOCKED_RECOVERY_AUDIT_DELTA")
    elif transition["reason"] == (
        "record_transport_uncertain_recovery_replay"
    ):
        allowed = {
        ...
        if (
            ...
            raise RuntimeError(
                "PUBLICATION_OWNER_TRANSPORT_RECOVERY_REPLAY_DELTA"
            )
    elif transition["reason"] == "no_journal_install_reactivation":
        allowed = {
            "recovery_results", "active_service_authorization",
            "active_authorization_sha256", "authorization_sequence",
            "pending_action", "transport_uncertain", "response_uncertain",
            "last_error", "last_preacquisition_status",
        }
        if changed - allowed:
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_FORBIDDEN_DELTA")
...
def encode_owner_transition_entry(
    transition: Mapping[str, object],
    *,
    from_owner_record: bytes,
    to_owner_record: bytes,
    ...
    validate_owner_transition_against_records(
        transition,
        from_owner_record=from_owner_record,
        to_owner_record=to_owner_record,
        expected_authorization=expected_authorization,
        creates_authorization=creates_authorization,
    )
    entry = {
        "transition": dict(transition),
        "from_owner_record": json.loads(from_owner_record),
        "to_owner_record": json.loads(to_owner_record),
    }
    validate_owner_transition_entry(entry)
    return entry


def validate_owner_transition_entry(
    entry: object,
) -> dict[str, object]:
    if type(entry) is not dict or set(entry) != OWNER_TRANSITION_ENTRY_KEYS:
        raise RuntimeError("PUBLICATION_OWNER_TRANSITION_ENTRY_SCHEMA")
    ...
    before_bytes = encode_authorization_owner_record(entry["from_owner_record"])
    after_bytes = encode_authorization_owner_record(entry["to_owner_record"])
    after = json.loads(after_bytes)
    expected_authorization = _active_owner_authorization(after)
    if expected_authorization is None:
        raise RuntimeError("PUBLICATION_OWNER_TRANSITION_TARGET")
    validate_owner_transition_against_records(
        transition,
        from_owner_record=before_bytes,
        to_owner_record=after_bytes,
        expected_authorization=expected_authorization,
        creates_authorization=transition["reason"] in {
            "activate_authorization",
            "activate_recovery_after_locked_install_response",
            "activate_recovery_after_transport_uncertain_install",
        },
...
def decode_owner_transition_transaction_journal(
    payload: bytes,
) -> tuple[tuple[dict[str, object], ...], dict[str, object] | None]:
    """Return committed entries and at most one recoverable PREPARED tail."""
    if payload == b"":
        return (), None
    if not payload.endswith(b"\n"):
        raise RuntimeError("OWNER_TRANSITION_TXN_TERMINATOR")
    encoded_frames = payload[:-1].split(b"\n")
    committed: list[dict[str, object]] = []
    pending: dict[str, object] | None = None
    previous_frame_sha256 = None
    for sequence, encoded in enumerate(encoded_frames, 1):
        frame = json.loads(encoded)
        if (
            type(frame) is not dict
            or set(frame) != OWNER_TRANSITION_TRANSACTION_FRAME_KEYS
            or _canonical_json(frame) != encoded
            or frame["schema_version"] != 1
            or frame["sequence"] != sequence
            or frame["previous_frame_sha256"] != previous_frame_sha256
        ):
            raise RuntimeError("OWNER_TRANSITION_TXN_SCHEMA")
        if frame["phase"] == "PREPARED":
            if pending is not None or frame["prepared_frame_sha256"] is not None:
                raise RuntimeError("OWNER_TRANSITION_TXN_PREPARED_ORDER")
            entry = validate_owner_transition_entry(frame["prepared_entry"])
            before_bytes = encode_authorization_owner_record(
                entry["from_owner_record"]
            )
            after_bytes = encode_authorization_owner_record(
                entry["to_owner_record"]
            )
            if (
                frame["transaction_id"]
                != entry["transition"]["transition_sha256"]
                or frame["before_owner_sha256"]
                != hashlib.sha256(before_bytes).hexdigest()
                or frame["after_owner_sha256"]
                != hashlib.sha256(after_bytes).hexdigest()
            ):
                raise RuntimeError("OWNER_TRANSITION_TXN_BINDING")
            pending = frame
        elif frame["phase"] == "COMMITTED":
            if (
                pending is None
                or frame["prepared_entry"] is not None
                or frame["transaction_id"] != pending["transaction_id"]
                or frame["before_owner_sha256"]
                != pending["before_owner_sha256"]
                or frame["after_owner_sha256"]
                != pending["after_owner_sha256"]
                or frame["prepared_frame_sha256"]
                != hashlib.sha256(
                    _canonical_json(pending)
                ).hexdigest()
            ):
                raise RuntimeError("OWNER_TRANSITION_TXN_COMMIT_ORDER")
            committed.append(pending["prepared_entry"])
            pending = None
        else:
            raise RuntimeError("OWNER_TRANSITION_TXN_PHASE")
        previous_frame_sha256 = hashlib.sha256(encoded).hexdigest()
    return tuple(committed), pending


def append_owner_transition_transaction_frame(
    existing: bytes,
    *,
    phase: str,
    transition_entry: Mapping[str, object] | None,
) -> bytes:
    committed, pending = decode_owner_transition_transaction_journal(existing)
    encoded_frames = existing[:-1].split(b"\n") if existing else []
    if phase == "PREPARED":
        if pending is not None or transition_entry is None:
            raise RuntimeError("OWNER_TRANSITION_TXN_NEXT_PHASE")
        entry = validate_owner_transition_entry(transition_entry)
        before_bytes = encode_authorization_owner_record(
            entry["from_owner_record"]
        )
        after_bytes = encode_authorization_owner_record(
        ...
        transaction_id = entry["transition"]["transition_sha256"]
        prepared_entry: Mapping[str, object] | None = entry
        prepared_frame_sha256 = None
    elif phase == "COMMITTED":
        if pending is None or transition_entry is not None:
            raise RuntimeError("OWNER_TRANSITION_TXN_NEXT_PHASE")
        before_bytes = encode_authorization_owner_record(
            pending["prepared_entry"]["from_owner_record"]
        )
        after_bytes = encode_authorization_owner_record(
            pending["prepared_entry"]["to_owner_record"]
        )
        transaction_id = pending["transaction_id"]
        prepared_entry = None
        prepared_frame_sha256 = hashlib.sha256(
            _canonical_json(pending)
        ).hexdigest()
    else:
        raise RuntimeError("OWNER_TRANSITION_TXN_PHASE")
    frame = {
        "schema_version": 1,
        "sequence": len(encoded_frames) + 1,
        "phase": phase,
        "transaction_id": transaction_id,
        "prepared_entry": prepared_entry,
        "before_owner_sha256": hashlib.sha256(before_bytes).hexdigest(),
        "after_owner_sha256": hashlib.sha256(after_bytes).hexdigest(),
        "prepared_frame_sha256": prepared_frame_sha256,
        "previous_frame_sha256": (
            None
            if not encoded_frames
            else hashlib.sha256(encoded_frames[-1]).hexdigest()
        ),
    }
    candidate = existing + _canonical_json(frame) + b"\n"
    next_committed, next_pending = (
        decode_owner_transition_transaction_journal(candidate)
    )
    if phase == "PREPARED":
        if len(next_committed) != len(committed) or next_pending is None:
            raise RuntimeError("OWNER_TRANSITION_TXN_POSTCONDITION")
    elif len(next_committed) != len(committed) + 1 or next_pending is not None:
        raise RuntimeError("OWNER_TRANSITION_TXN_POSTCONDITION")
    return candidate
...
def encode_owner_transition(
    *,
    ...
    body = {
        "from_authorization_id": from_authorization_id,
        "from_authorization_sequence": from_authorization_sequence,
        "from_owner_record_sha256": from_owner_record_sha256,
        "from_pending_action": from_pending_action,
        "previous_transition_sha256": (
            None
            if previous_transition is None
            else previous_transition["transition_sha256"]
        ),
        "reason": reason,
        ...
        "transition_sequence": (
            1
            if previous_transition is None
            else previous_transition["transition_sequence"] + 1
        ),
    }
    return {
        **body,
        "transition_sha256": hashlib.sha256(_canonical_json(body)).hexdigest(),
    }


def _authorization_digest_document(
    authorization: Mapping[str, object],
) -> dict[str, object]:
    return {
        key: authorization[key]
        for key in set(authorization) - {"authorization_sha256"}
    }
...
def _authorization_request_fields(
    ...
    construction_kind = (
        "base_python" if request["operation"].endswith("base_python")
        else "closure"
    )
    return {
        ...
        "descriptor_or_intake_sha256": payload[
            "descriptor_or_intake_sha256"
        ],
        "construction_attempt": payload["construction_attempt"],
        "expected_source_kind": payload["expected_source_kind"],
        "expected_destination_kind": payload["expected_destination_kind"],
        "owner_token_sha256": payload["owner_token_sha256"],
        "operation_token_sha256": payload["operation_token_sha256"],
    }


def encode_pending_publication_authorization(
    *,
    request: Mapping[str, object],
    authorization_sequence: int,
    ...
    payload = request["payload"]
    generation = payload["recovery_generation"]
    kind = "install" if generation is None else "recovery"
    if (kind == "install") != (previous_authorization is None):
...
def encode_publication_authorization(
    *,
    request: Mapping[str, object],
    authorization_sequence: int,
    previous_authorization: Mapping[str, object] | None,
    owner_transition: Mapping[str, object],
    recovery_reason: str | None = None,
) -> dict[str, object]:
    authorization = encode_pending_publication_authorization(
        request=request,
        authorization_sequence=authorization_sequence,
        previous_authorization=previous_authorization,
        recovery_reason=recovery_reason,
    )
    authorization["owner_transition"] = dict(owner_transition)
    authorization["authorization_sha256"] = hashlib.sha256(
        _canonical_json(_authorization_digest_document(authorization))
    ).hexdigest()
    return authorization
...
def _validate_authorization_shape(
    ...
    *,
    ...
    if type(authorization) is not dict:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_AUTHORIZATION_SCHEMA")
    kind = authorization.get("kind")
    expected_keys = (
        INSTALL_AUTHORIZATION_KEYS
        if kind == "install" else RECOVERY_AUTHORIZATION_KEYS
    )
    if kind not in {"install", "recovery"} or set(authorization) != expected_keys:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_AUTHORIZATION_SCHEMA")
    for key in (
        "authorization_id", "operation", "construction_kind", "source_id",
        "expected_source_kind", "expected_destination_kind",
    ):
        if type(authorization[key]) is not str or not authorization[key]:
            raise RuntimeError("PUBLICATION_OWNER_RECORD_AUTHORIZATION_VALUE")
    if (
        type(authorization["authorization_sequence"]) is not int
        or authorization["authorization_sequence"] <= 0
    ):
        raise RuntimeError("PUBLICATION_OWNER_RECORD_AUTHORIZATION_VALUE")
    for key in (
        "request_sha256", "profile_sha256", "descriptor_or_intake_sha256",
        "owner_token_sha256", "operation_token_sha256",
    ):
        _require_sha256(
            authorization[key], code="PUBLICATION_OWNER_RECORD_AUTHORIZATION_DIGEST"
        )
    if authorization["construction_kind"] == "base_python":
        if (
            authorization["destination_id"] is not None
            or type(authorization["construction_attempt"]) is not int
            or authorization["construction_attempt"] <= 0
            or not authorization["operation"].endswith("base_python")
        ):
            raise RuntimeError("PUBLICATION_AUTHORIZATION_CONSTRUCTION")
    elif authorization["construction_kind"] == "closure":
        if (
            type(authorization["destination_id"]) is not str
            or not authorization["destination_id"]
            or authorization["construction_attempt"] is not None
            or not authorization["operation"].endswith("closure")
        ):
            raise RuntimeError("PUBLICATION_AUTHORIZATION_CONSTRUCTION")
    else:
        raise RuntimeError("PUBLICATION_AUTHORIZATION_CONSTRUCTION")
    if authorization["owner_transition"] is not None and type(
        authorization["owner_transition"]
    ) is not dict:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_AUTHORIZATION_VALUE")
    if kind == "install":
        ...
        if (
            authorization["operation"] not in {
                "recover_base_python", "recover_closure",
            }
            or type(authorization["recovery_generation"]) is not int
            or authorization["recovery_generation"] <= 0
            or type(authorization["reason"]) is not str
    ...
    if allow_pending and digest is None and authorization["owner_transition"] is None:
        return authorization
    _require_sha256(digest, code="PUBLICATION_OWNER_RECORD_AUTHORIZATION_DIGEST")
    if authorization["owner_transition"] is None:
        raise RuntimeError("PUBLICATION_OWNER_RECORD_AUTHORIZATION_VALUE")
    expected = hashlib.sha256(
        _canonical_json(_authorization_digest_document(authorization))
    ).hexdigest()
    if digest != expected:
        raise RuntimeError("PUBLICATION_AUTHORIZATION_DIGEST")
    return authorization


def _validate_owner_authorization_histories(
    owner: Mapping[str, object], *, allow_pending_authorization: bool
) -> None:
    install = owner["install_authorization"]
    recoveries = owner["recovery_authorizations"]
    results = owner["recovery_results"]
    if install is not None:
        _validate_authorization_shape(
            install, allow_pending=allow_pending_authorization
        )
    ...
    for generation, result in enumerate(results, 1):
        if type(result) is not dict or set(result) != RECOVERY_RESULT_KEYS:
            raise RuntimeError("PUBLICATION_OWNER_RECOVERY_RESULT_SCHEMA")
        authorization = recoveries[generation - 1]
        if (
            result["recovery_generation"] != generation
            or result["authorization_sha256"]
            != authorization["authorization_sha256"]
            or result["request_sha256"] != authorization["request_sha256"]
            or result["previous_result_sha256"]
            != (None if previous_result is None else previous_result["result_sha256"])
            or result["terminal_phase"] not in {
                "NO_JOURNAL_NO_MUTATION", "ABORTED_BEFORE_PREPARED",
                "BASE_COMPLETE", "COMPLETE",
            }
        ):
            raise RuntimeError("PUBLICATION_OWNER_RECOVERY_RESULT_CHAIN")
        for key in (
            "authorization_sha256", "request_sha256",
            "terminal_response_sha256", "transaction_lock_identity_sha256",
            "result_sha256",
        ):
            _require_sha256(
                result[key], code="PUBLICATION_OWNER_RECOVERY_RESULT_DIGEST"
            )
        if result["result_sha256"] != hashlib.sha256(
            _canonical_json(_result_digest_document(result))
        ).hexdigest():
            raise RuntimeError("PUBLICATION_OWNER_RECOVERY_RESULT_DIGEST")
        previous_result = result
    active = owner["active_service_authorization"]
    expected_result_count = len(recoveries)
    if active is not None and active["kind"] == "recovery":
        if active["generation"] != len(recoveries):
            raise RuntimeError("PUBLICATION_OWNER_ACTIVE_AUTHORIZATION_REFERENCE")
...
def _changed_owner_keys(
    previous: Mapping[str, object], current: Mapping[str, object]
) -> set[str]:
    return {key for key in OWNER_RECORD_KEYS if previous[key] != current[key]}


def _validate_owner_heartbeat_order(
...
def _validate_permitted_owner_transition_gap(
    ...
    """Validate one omitted owner replacement by its exact reason.

    Authorization-transition history may omit a high-rate heartbeat replacement
    or a same-kind forward-only phase replacement owned by the lifecycle state
    machine.  The zero-FD transaction-busy observation is written to a separate
    append-only diagnostic and never replaces the owner record. Cleanup, pending
    action, uncertainty, response, result, and publication-mirror changes are
    never projections: their owning state machine must persist and validate them.
    """
    changed = _changed_owner_keys(previous_after, current_before)
    if changed <= OWNER_HEARTBEAT_KEYS:
        _validate_owner_heartbeat_order(previous_after, current_before)
        return "heartbeat"
    if changed <= OWNER_PHASE_GAP_KEYS and "phase" in changed:
        kind = previous_after["construction_kind"]
        if current_before["construction_kind"] != kind:
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_PHASE_KIND")
        phases = OWNER_PHASES_BY_KIND.get(str(kind), ())
        try:
            previous_index = phases.index(str(previous_after["phase"]))
            current_index = phases.index(str(current_before["phase"]))
        except ValueError as error:
            raise RuntimeError(
                "PUBLICATION_OWNER_TRANSITION_PHASE_KIND"
            ) from error
        if current_index <= previous_index:
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_PHASE_ORDER")
        _validate_owner_heartbeat_order(previous_after, current_before)
        return "phase_progress"
    raise RuntimeError("PUBLICATION_OWNER_TRANSITION_RECORD_CHAIN")


def validate_owner_history_for_service(
    entries: Sequence[Mapping[str, object]],
    *,
    current_owner_record: Mapping[str, object],
) -> tuple[dict[str, object], ...]:
    """Bind service authorization to history plus a heartbeat-only live tail."""
    transitions, last_target = _validate_owner_transition_history(
        entries, include_last_target=True
    )
    current = _validate_owner_record_mapping(current_owner_record)
    changed = _changed_owner_keys(last_target, current)
    if not changed <= OWNER_HEARTBEAT_KEYS:
        raise RuntimeError("PUBLICATION_OWNER_SERVICE_TAIL")
    try:
        _validate_owner_heartbeat_order(last_target, current)
    except RuntimeError as error:
        raise RuntimeError("PUBLICATION_OWNER_SERVICE_TAIL") from error
    return transitions
...
def _validate_owner_transition_history(
    entries: Sequence[Mapping[str, object]],
    *,
    include_last_target: bool = False,
) -> tuple[dict[str, object], ...] | tuple[
    tuple[dict[str, object], ...], dict[str, object]
]:
    previous: Mapping[str, object] | None = None
    previous_after: Mapping[str, object] | None = None
    transitions: list[dict[str, object]] = []
    for sequence, entry in enumerate(entries, 1):
        validated_entry = validate_owner_transition_entry(entry)
        transition = validated_entry["transition"]
        for key in (
            "from_owner_record_sha256",
            "to_owner_record_projection_sha256",
            "transition_sha256",
            ...
            _require_sha256(
                transition[key], code="PUBLICATION_OWNER_TRANSITION_DIGEST"
            )
        if (
            type(transition["transition_sequence"]) is not int
            or transition["transition_sequence"] <= 0
            or type(transition["from_authorization_sequence"]) is not int
            or transition["from_authorization_sequence"] < 0
            or type(transition["to_authorization_sequence"]) is not int
            or transition["to_authorization_sequence"] <= 0
            or type(transition["to_authorization_id"]) is not str
            or not transition["to_authorization_id"]
            or type(transition["from_pending_action"]) is not str
            or type(transition["to_pending_action"]) is not str
            or type(transition["reason"]) is not str
            or not transition["reason"]
        ):
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_VALUE")
        body = {
            key: transition[key]
            for key in OWNER_TRANSITION_KEYS - {"transition_sha256"}
        }
        expected_digest = hashlib.sha256(_canonical_json(body)).hexdigest()
        if transition["transition_sha256"] != expected_digest:
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_DIGEST")
        if (
            transition["transition_sequence"] != sequence
            or transition["previous_transition_sha256"]
            != (None if previous is None else previous["transition_sha256"])
            or transition["to_authorization_sequence"]
            != transition["from_authorization_sequence"] + (
                0
            ...
            raise RuntimeError("PUBLICATION_OWNER_TRANSITION_CHAIN")
    ...
    if previous_after is None:
        raise RuntimeError("PUBLICATION_OWNER_TRANSITION_HISTORY_REQUIRED")
    validated = tuple(transitions)
    return (validated, dict(previous_after)) if include_last_target else validated
...
def _validate_publication_authorization(
    ...
    *,
    ...
    _validate_authorization_shape(authorization, allow_pending=False)
    payload = request["payload"]
    if type(payload) is not dict:
        raise RuntimeError("PUBLICATION_AUTHORIZATION_REQUEST")
    expected_request_fields = _authorization_request_fields(request)
    if any(
        authorization[key] != expected_request_fields[key]
        for key in expected_request_fields
    ):
        raise RuntimeError("PUBLICATION_AUTHORIZATION_REQUEST_BINDING")
    ...
    if transition["transition_sha256"] != hashlib.sha256(
    ...
    initial_source_valid = (
        transition["from_authorization_id"] is None
        and transition["from_authorization_sequence"] == 0
    )
    subsequent_source_valid = (
        type(transition["from_authorization_id"]) is str
        and bool(transition["from_authorization_id"])
        and type(transition["from_authorization_sequence"]) is int
        and transition["from_authorization_sequence"] > 0
    )
    if (
        not (
...
def _journal_actor_fields(
    ...
    return {
        ...
        "actor_owner_transition_sha256": transition["transition_sha256"],
        "actor_previous_authorization_entry_sha256": (
            None if authorization["kind"] == "install"
            else authorization["previous_entry_sha256"]
        ),
...
def _expected_next_publication_phase(
    ...
    for record in records:
        if record["phase"] == "RECOVERY_STARTED":
            ...
            recovery_validated = False
        elif record["phase"] in PRE_PREPARED_ABORT_PHASES:
            abort_count += 1
            if record["phase"] == "ABORTED_BEFORE_PREPARED":
                active_recovery = False
    ...
    if not records:
        return "INITIAL"
    if records[-1]["phase"] == "ABORTED_BEFORE_PREPARED":
        return None
    if active_recovery:
        if install_count == 1:
            return PRE_PREPARED_ABORT_PHASES[abort_count]
...
def append_publication_phase(
    ...
    *,
    ...
    adopted_install_response: bytes | None = None,
    owner_transition_history: Sequence[Mapping[str, object]] = (),
) -> bytes:
    actor_already_authorized = actor_request["request_id"] in {
        request["request_id"] for request in recovery_requests
    }
    existing_transition_history = tuple(owner_transition_history)
    if (
        phase == "RECOVERY_STARTED"
        and not actor_already_authorized
        and existing_transition_history
        and existing_transition_history[-1]["transition"]
        == actor_authorization["owner_transition"]
    ):
        existing_transition_history = existing_transition_history[:-1]
    records = decode_exact_publication_journal(
        existing,
        install_request=install_request,
        install_authorization=install_authorization,
        recovery_requests=recovery_requests,
        recovery_authorizations=recovery_authorizations,
        owner_transition_history=existing_transition_history,
        allow_empty=True,
    )
    expected_phase = _expected_next_publication_phase(records)
    if phase == "RECOVERY_STARTED":
        if records and records[-1]["phase"] == "ABORTED_BEFORE_PREPARED":
            raise RuntimeError("PUBLICATION_ABORTED_ATTEMPT_TERMINAL")
    ...
    elif phase == RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED:
        if (
            actor_request is install_request
            or expected_phase != "RECEIPT_CREATED"
        ):
            raise RuntimeError("PUBLICATION_RECOVERY_ADOPTION_PHASE")
    ...
    receipt_created = any(
        record["phase"] in {
            "RECEIPT_CREATED", RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED,
        }
        for record in records
    ) or phase in {
        "RECEIPT_CREATED", RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED,
    ...
    recovery_terminal = phase in {
        "ABORTED_BEFORE_PREPARED", "RECOVERY_RESPONSE_CREATED",
    }
    ...
    adoption = phase == RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED
    if adoption != (adopted_install_response is not None):
        raise RuntimeError("PUBLICATION_JOURNAL_ADOPTION_RESPONSE")
    ...
    record = {
        ...
        **result,
        ...
        "response_operation_identity": _stable_publication_identity(
            publication.get(
                "response_operation_identity",
                publication["source_parent_identity"],
            )
        ...
        "adopted_install_request_id": (
            install_request["request_id"] if adoption else None
        ),
        "adopted_install_response_sha256": (
            hashlib.sha256(adopted_install_response).hexdigest()
            if adopted_install_response is not None else None
        ),
    }
    ...
    if phase == "RECOVERY_STARTED" and not actor_already_authorized:
...
def decode_exact_publication_journal(
    ...
    *,
    ...
    if not owner_transition_history:
        raise RuntimeError("PUBLICATION_OWNER_TRANSITION_HISTORY_REQUIRED")
    transitions = _validate_owner_transition_history(owner_transition_history)
    transition_digests = {
    ...
    known_authorizations = {
        ...
        for authorization in (install_authorization, *recovery_authorizations)
    }
    install_authorization_id = install_authorization["authorization_id"]
    for transition in transitions:
        target = (
            transition["to_authorization_id"],
            transition["to_authorization_sequence"],
        )
        if transition["reason"] == "no_journal_install_reactivation":
            if (
                transition["to_authorization_id"] != install_authorization_id
                or transition["to_pending_action"] != install_request["operation"]
            ):
                raise RuntimeError("PUBLICATION_OWNER_REACTIVATION_TARGET")
        else:
            if transition["reason"] not in {
                ...
                raise RuntimeError("PUBLICATION_OWNER_TRANSITION_REASON")
            if target not in known_authorizations:
                raise RuntimeError("PUBLICATION_OWNER_TRANSITION_TARGET")
    for authorization in (
        install_authorization, *recovery_authorizations,
    ):
        if authorization["owner_transition"]["transition_sha256"] not in (
            transition_digests
        ):
            raise RuntimeError("PUBLICATION_AUTHORIZATION_TRANSITION_MISSING")
    expected_install = _install_publication_fields(install_request)
    previous_entry = None
    records: list[dict[str, object]] = []
    install_count = 0
    active_recovery_index: int | None = None
    terminal_actor_index: int | None = None
    encountered_recoveries: set[int] = set()
    recovery_validated = False
    pre_prepared_abort = False
    abort_count = 0
    prepared = False
    ...
    for sequence, encoded in enumerate(encoded_records, 1):
        ...
        if phase == "RECOVERY_STARTED":
            ...
            matching = [
                index
                for index, request in enumerate(recovery_requests)
                if request["request_id"] == record["actor_request_id"]
            ]
            if len(matching) != 1:
                raise RuntimeError("PUBLICATION_RECOVERY_UNDECLARED")
            active_recovery_index = matching[0]
            if active_recovery_index in encountered_recoveries:
                raise RuntimeError("PUBLICATION_RECOVERY_REUSED")
            encountered_recoveries.add(active_recovery_index)
            recovery_validated = False
            pre_prepared_abort = install_count == 1
            abort_count = 0
        elif pre_prepared_abort:
            expected = PRE_PREPARED_ABORT_PHASES[abort_count]
            if phase != expected:
                raise RuntimeError("PUBLICATION_PREPARED_ABORT_ORDER")
            abort_count += 1
            if phase == "ABORTED_BEFORE_PREPARED":
                terminal_actor_index = active_recovery_index
            ...
            if (
                phase != expected
                and not (
                    expected == "RECEIPT_CREATED"
                    and phase == RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED
                )
            ):
            ...
            active_recovery_index = None
            recovery_validated = False
        if active_recovery_index is None and phase not in {
            "ABORTED_BEFORE_PREPARED", "RECOVERY_RESPONSE_CREATED",
        }:
            actor_request = install_request
            actor_authorization = install_authorization
        ...
        identities = (
            ...
            record["response_operation_identity"],
        )
        ...
        terminal_expected = (
            phase in {"ABORTED_BEFORE_PREPARED", "RECOVERY_RESPONSE_CREATED"}
        ...
        if phase == RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED:
            if (
                actor_request is install_request
                or record["adopted_install_request_id"]
                != install_request["request_id"]
            ):
                raise RuntimeError("PUBLICATION_RECOVERY_ADOPTION_ACTOR")
            _require_sha256(
                record["adopted_install_response_sha256"],
                code="PUBLICATION_RECOVERY_ADOPTION_DIGEST",
            )
        elif (
            record["adopted_install_request_id"] is not None
            or record["adopted_install_response_sha256"] is not None
        ):
            raise RuntimeError("PUBLICATION_JOURNAL_EARLY_ADOPTION")
        records.append(record)
...
def _fd_mount_id(fd: int) -> int:
    with open(f"/proc/self/fdinfo/{fd}", encoding="ascii") as stream:
        for line in stream:
            if line.startswith("mnt_id:"):
                return int(line.split(":", 1)[1])
    raise RuntimeError("PROTECTED_RESPONSE_MOUNT_ID")


def read_protected_response_artifact(
    parent_fd: int,
    *,
    request_id: str,
    ...
    """Read one root-created response artifact through its pinned parent FD."""
    _validate_publication_identity(
        expected_parent_identity, code="PROTECTED_RESPONSE_PARENT_IDENTITY"
    )
    parent_status = os.fstat(parent_fd)
    observed_parent = {
        "device": parent_status.st_dev,
        "inode": parent_status.st_ino,
        "mount_id": _fd_mount_id(parent_fd),
        "mode": stat.S_IMODE(parent_status.st_mode),
        "uid": parent_status.st_uid,
        "gid": parent_status.st_gid,
    }
    if observed_parent != expected_parent_identity:
        raise RuntimeError("PROTECTED_RESPONSE_PARENT_IDENTITY")
    basename = f"{hashlib.sha256(request_id.encode()).hexdigest()}.bin"
    fd = os.open(
        basename,
        os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW,
    ...
    try:
        before = os.fstat(fd)
        before_identity = (
            before.st_dev, before.st_ino, _fd_mount_id(fd), before.st_mode,
        ...
        if (
            ...
            or _fd_mount_id(fd) != expected_parent_identity["mount_id"]
        ):
            raise RuntimeError("PROTECTED_RESPONSE_FILE_IDENTITY")
        chunks: list[bytes] = []
        while True:
        ...
        after = os.fstat(fd)
        after_identity = (
            after.st_dev, after.st_ino, _fd_mount_id(fd), after.st_mode,
            after.st_uid, after.st_gid, after.st_nlink, after.st_size,
        )
        if after_identity != before_identity:
            raise RuntimeError("PROTECTED_RESPONSE_FILE_CHANGED")
    finally:
        os.close(fd)
    decode_exact_response_payload(response_bytes, request_id=request_id)
    return response_bytes


def validate_durable_installer_response(
    *,
    response_bytes: bytes,
    request: Mapping[str, object],
    recovery_authorization: Mapping[str, object] | None = None,
    recovery_result: Mapping[str, object] | None = None,
) -> "DecodedInstallerResponse":
    """Shared byte-level validator for receive, restart, and publication paths."""
    request_id = request["request_id"]
    if type(request_id) is not str or not request_id:
        raise RuntimeError("PROTECTED_RESPONSE_REQUEST_ID")
    response = decode_exact_response_payload(
        response_bytes, request_id=request_id
    )
    ...
    validate_reply_for_request(reply, request)
    if (recovery_authorization is None) != (recovery_result is None):
        raise RuntimeError("PROTECTED_RESPONSE_RESULT_PAIR")
    if recovery_result is not None and recovery_authorization is not None:
        if set(recovery_result) != RECOVERY_RESULT_KEYS:
            raise RuntimeError("PROTECTED_RESPONSE_RESULT_SCHEMA")
        expected = {
            "recovery_generation": recovery_authorization[
                "recovery_generation"
            ],
            ...
            "request_sha256": recovery_authorization["request_sha256"],
            "terminal_response_sha256": response.sha256,
            "terminal_phase": response.payload["terminal_phase"],
        }
        if any(recovery_result[key] != value for key, value in expected.items()):
            raise RuntimeError("PROTECTED_RESPONSE_RESULT_BINDING")
        _require_sha256(
            recovery_result["result_sha256"],
            code="PROTECTED_RESPONSE_RESULT_DIGEST",
        )
        if recovery_result["result_sha256"] != hashlib.sha256(
            _canonical_json(_result_digest_document(recovery_result))
        ).hexdigest():
            raise RuntimeError("PROTECTED_RESPONSE_RESULT_DIGEST")
    return response


PUBLICATION_OWNER_MUTABLE_KEYS = {
    "state", "phase", "attempt_status", "cleanup_required",
    "pending_action", "pending_local_transition",
    "heartbeat_monotonic_ns", "heartbeat_wall_ns", "last_error",
    "recovery_results", "active_service_authorization",
    "active_authorization_sha256", "authorization_sequence",
    "transport_uncertain",
    "response_uncertain", "locked_response_records",
    "publication_mirrors", "protected_journal_validated",
    "protected_receipt_validated", "published_root_validated",
    "component_hashes", "final_tree_sha256", "receipt_sha256",
    "terminal_response_status",
}
PUBLICATION_OWNER_STAGES = {
    "base_python": {
        "BASE_METADATA_NORMALIZED": ((), False, False, True),
        "BASE_FINAL_RENAMED": (("BASE_FINAL_RENAMED",), True, False, True),
        "BASE_RECEIPT_CREATED": (
            ("BASE_FINAL_RENAMED", "BASE_RECEIPT_CREATED"), True, False, True,
        ),
        "BASE_COMPLETE": (
            ("BASE_FINAL_RENAMED", "BASE_RECEIPT_CREATED"), True, True, False,
        ),
    },
    "closure": {
        "INSTALLING": ((), False, False, True),
        "FINAL_RENAMED": (("FINAL_RENAMED",), True, False, True),
        "RECEIPT_CREATED": (
            ("FINAL_RENAMED", "RECEIPT_CREATED"), True, False, True,
        ),
        "COMPLETE": (
            ("FINAL_RENAMED", "RECEIPT_CREATED"), True, True, False,
        ),
    },
}


def validate_current_owner_for_publication(
    *,
    last_authorization_target: Mapping[str, object],
    current_owner_record: Mapping[str, object],
    recovery_results: Sequence[Mapping[str, object]],
    latest_response: "DecodedInstallerResponse",
) -> dict[str, object]:
    """Bind every owner field before a publication mirror is authorized."""
    previous = _validate_owner_record_mapping(last_authorization_target)
    current = _validate_owner_record_mapping(current_owner_record)
    for key in OWNER_RECORD_KEYS - PUBLICATION_OWNER_MUTABLE_KEYS:
        if current[key] != previous[key]:
            raise RuntimeError("PROTECTED_RESPONSE_OWNER_IMMUTABLE_FIELD")
    _validate_owner_heartbeat_order(previous, current)
    if current["authorization_sequence"] != (
        previous["authorization_sequence"] + 1
    ):
        raise RuntimeError("PROTECTED_RESPONSE_OWNER_TERMINAL_SEQUENCE")
    authorizations = _owner_authorizations(current)
    locked_authorizations = {
        authorization["request_sha256"]: (
            authorization["authorization_sha256"],
            authorization.get("recovery_generation"),
    ...
    locked_records = validate_locked_response_history(
        current["locked_response_records"],
        authorization_by_request_sha256=locked_authorizations,
        required_prefix=previous["locked_response_records"],
    )
    kind = str(current["construction_kind"])
    stage = PUBLICATION_OWNER_STAGES.get(kind, {}).get(str(current["phase"]))
    if stage is None:
        raise RuntimeError("PROTECTED_RESPONSE_OWNER_PHASE")
    mirrors, protected_inputs, published_root, cleanup_required = stage
    complete = not cleanup_required
    expected = {
        "state": "COMPLETE" if complete else "INSTALLING",
        "attempt_status": (
            "complete" if complete else "active"
        ) if kind == "base_python" else None,
        "cleanup_required": cleanup_required,
        "pending_action": None,
        "pending_local_transition": None if complete else "VALIDATE_PUBLICATION",
        "last_error": None,
        "recovery_results": list(recovery_results),
        "active_service_authorization": None,
        "active_authorization_sha256": None,
        "transport_uncertain": False,
        "response_uncertain": False,
        "locked_response_records": list(locked_records),
        "publication_mirrors": list(mirrors),
        "protected_journal_validated": protected_inputs,
        "protected_receipt_validated": protected_inputs,
        "published_root_validated": published_root,
        "component_hashes": latest_response.payload["component_hashes"],
        "final_tree_sha256": latest_response.payload["final_tree_sha256"],
        "receipt_sha256": latest_response.payload["receipt_sha256"],
        "terminal_response_status": latest_response.status,
    }
    for key, value in expected.items():
        if current[key] != value:
            raise RuntimeError("PROTECTED_RESPONSE_OWNER_LIFECYCLE_FIELD")
    return current


def validate_protected_publication_contract(
    *,
    ...
    """Validate journal, exact response bytes, results, receipt, and owner."""
    owner = _validate_owner_record_mapping(current_owner_record)
    if (
        owner["install_authorization"] != install_authorization
        or owner["recovery_authorizations"] != list(recovery_authorizations)
        or owner["recovery_results"] != list(recovery_results)
    ):
        raise RuntimeError("PROTECTED_RESPONSE_OWNER_HISTORY")
    _, last_authorization_target = _validate_owner_transition_history(
        owner_transition_history, include_last_target=True
    )
    records = decode_exact_publication_journal(
        journal_payload,
        install_request=install_request,
    ...
    receipt = decode_exact_publication_receipt(
        receipt_payload, install_request=install_request
    )
    request_by_id = {
        request["request_id"]: request
        for request in (install_request, *recovery_requests)
    }
    authorization_by_generation = {
        authorization["recovery_generation"]: authorization
        for authorization in recovery_authorizations
    }
    result_by_generation = {
        result["recovery_generation"]: result for result in recovery_results
    }
    terminal_records = [
        record for record in records
        if record["terminal_response_sha256"] is not None
    ]
    if not terminal_records:
        raise RuntimeError("PROTECTED_RESPONSE_MISSING")
    journal_request_ids = {
        str(record["actor_request_id"]) for record in terminal_records
    }
    adoption_records = [
        record for record in records
        if record["adopted_install_response_sha256"] is not None
    ]
    if len(adoption_records) > 1:
        raise RuntimeError("PROTECTED_RESPONSE_ADOPTION_COUNT")
    adoption_by_request = {
        str(record["adopted_install_request_id"]): record
        for record in adoption_records
    }
    journal_request_ids.update(adoption_by_request)
    recovery_request_by_generation = {
        request["payload"]["recovery_generation"]: request
        for request in recovery_requests
    }
    result_request_ids: set[str] = set()
    for result in recovery_results:
        request = recovery_request_by_generation.get(
            result["recovery_generation"]
        )
        if request is None:
            raise RuntimeError("PROTECTED_RESPONSE_RESULT_REQUEST")
        result_request_ids.add(str(request["request_id"]))
    required_request_ids = journal_request_ids | result_request_ids
    if set(response_artifacts) != required_request_ids:
        raise RuntimeError("PROTECTED_RESPONSE_ARTIFACT_SET")
    response_by_request: dict[str, DecodedInstallerResponse] = {}
    terminal_record_by_request = {
        str(record["actor_request_id"]): record for record in terminal_records
    }
    for request_id in required_request_ids:
        ...
        authorization = (
            ...
            else authorization_by_generation.get(generation)
        )
        result = None if generation is None else result_by_generation.get(generation)
        response = validate_durable_installer_response(
            response_bytes=response_artifacts[request_id],
            request=request,
            recovery_authorization=authorization,
            recovery_result=result,
        )
        record = terminal_record_by_request.get(request_id)
        if record is None:
            adoption = adoption_by_request.get(request_id)
            if adoption is not None:
                if (
                    response.status != "INSTALLED"
                    or response.sha256
                    != adoption["adopted_install_response_sha256"]
                ):
                    raise RuntimeError("PROTECTED_RESPONSE_ADOPTION_BINDING")
            elif (
                result is None
                or result["terminal_phase"] != "NO_JOURNAL_NO_MUTATION"
                or response.status != "NO_JOURNAL_NO_MUTATION"
            ):
                raise RuntimeError("PROTECTED_RESPONSE_UNJOURNALED_RESULT")
        elif response.sha256 != record["terminal_response_sha256"]:
            raise RuntimeError("PROTECTED_RESPONSE_JOURNAL_BINDING")
        if response.status in {"INSTALLED", "RECOVERED"}:
            expected_result = {
            ...
            if any(
                ...
                raise RuntimeError("PROTECTED_RESPONSE_RECEIPT_BINDING")
        response_by_request[request_id] = response
    latest_record = terminal_records[-1]
    latest_response = response_by_request[str(latest_record["actor_request_id"])]
    if latest_response.status not in {"INSTALLED", "RECOVERED"}:
        raise RuntimeError("PROTECTED_RESPONSE_PUBLICATION_RESULT")
    validate_current_owner_for_publication(
        last_authorization_target=last_authorization_target,
        current_owner_record=owner,
...
# ruff: noqa: F821
class OwnerTransitionTransactionMixin:
    """Restartable write-ahead owner/history transaction owned by the manager."""

    def persist_owner_transition_transaction(
        self,
        entry: Mapping[str, object],
        *,
        fault_hook: Callable[[str], None] | None = None,
    ) -> None:
        self.reconcile_owner_transition_transaction(fault_hook=fault_hook)
        validated = validate_owner_transition_entry(entry)
        before_bytes = encode_authorization_owner_record(
            validated["from_owner_record"]
        )
        after_bytes = encode_authorization_owner_record(
            validated["to_owner_record"]
        )
        if self.read_owner_record_bytes() != before_bytes:
            raise RuntimeError("OWNER_TRANSITION_TXN_OWNER_PRECONDITION")
        transaction_bytes = self.read_owner_transition_transaction_bytes()
        committed, pending = decode_owner_transition_transaction_journal(
            transaction_bytes
        )
        history = tuple(self.read_owner_transition_history_unreconciled())
        if pending is not None or history != committed:
            raise RuntimeError("OWNER_TRANSITION_TXN_HISTORY_PRECONDITION")

        prepared = append_owner_transition_transaction_frame(
            transaction_bytes,
            phase="PREPARED",
            transition_entry=validated,
        )
        self.replace_owner_transition_transaction_bytes(
            prepared,
            hook_prefix="owner_transition_prepared",
            fault_hook=fault_hook,
        )
        self.replace_owner_transition_history(
            (*history, validated),
            hook_prefix="owner_transition_history",
            fault_hook=fault_hook,
        )
        self.replace_owner_record_bytes(
            after_bytes,
            hook_prefix="owner_transition_owner",
            fault_hook=fault_hook,
        )
        committed_bytes = append_owner_transition_transaction_frame(
            prepared,
            phase="COMMITTED",
        ...
        self.replace_owner_transition_transaction_bytes(
            committed_bytes,
            hook_prefix="owner_transition_committed",
            fault_hook=fault_hook,
        )

    def reconcile_owner_transition_transaction(
        self,
        *,
        fault_hook: Callable[[str], None] | None = None,
    ) -> None:
        ...
        if pending is None:
            if history != committed:
                raise RuntimeError("OWNER_TRANSITION_TXN_COMMITTED_HISTORY")
            return
        entry = validate_owner_transition_entry(pending["prepared_entry"])
        before_bytes = encode_authorization_owner_record(
            entry["from_owner_record"]
        )
        after_bytes = encode_authorization_owner_record(
        ...
        if history == committed:
            if self.read_owner_record_bytes() != before_bytes:
                raise RuntimeError("OWNER_TRANSITION_TXN_RECOVERY_ORDER")
            self.replace_owner_transition_history(
                (*history, entry),
                hook_prefix="owner_transition_history",
                fault_hook=fault_hook,
        ...
        elif history != (*committed, entry):
            raise RuntimeError("OWNER_TRANSITION_TXN_RECOVERY_HISTORY")
        owner_bytes = self.read_owner_record_bytes()
        if owner_bytes == before_bytes:
            self.replace_owner_record_bytes(
                after_bytes,
                hook_prefix="owner_transition_owner",
                fault_hook=fault_hook,
            )
        elif owner_bytes != after_bytes:
            raise RuntimeError("OWNER_TRANSITION_TXN_RECOVERY_OWNER")
        committed_bytes = append_owner_transition_transaction_frame(
            transaction_bytes,
            phase="COMMITTED",
            transition_entry=None,
    ...
    def read_committed_owner_transition_history(
        self,
    ) -> tuple[dict[str, object], ...]:
        committed, pending = decode_owner_transition_transaction_journal(
            self.read_owner_transition_transaction_bytes()
        )
        if pending is not None:
            raise RuntimeError("OWNER_TRANSITION_TXN_RECONCILIATION_REQUIRED")
        history = tuple(self.read_owner_transition_history_unreconciled())
        if history != committed:
            raise RuntimeError("OWNER_TRANSITION_TXN_COMMITTED_HISTORY")
        return committed
...
class ProtectedResponseValidationMixin:
    """Concrete protected-response seam used by DiskAuthorizationStore."""

    protected_response_parent_fd: int
    protected_response_parent_identity: Mapping[str, object]
    protected_response_directory_journal_parent_fd: int
    protected_response_directory_journal_parent_identity: Mapping[str, object]
    protected_response_operation_token_sha256: str
    protected_response_uid: int
    protected_response_gid: int

    def read_service_evidence(self) -> dict[str, object]:
        """Reopen canonical manager files; never use caller resume objects."""
        owner = _decode_authorization_owner_record(
            self.read_owner_record_bytes()
        )
        install_request = decode_exact_installer_request(
            self.read_install_request_bytes()
        )
        recovery_requests = tuple(
            decode_exact_installer_request(payload)
            for payload in self.read_recovery_request_bytes()
        )
        if len(recovery_requests) != len(owner["recovery_authorizations"]):
            raise RuntimeError("INSTALLER_SERVICE_RECOVERY_REQUEST_COUNT")
        transitions = self.read_committed_owner_transition_history()
        validate_owner_history_for_service(
            transitions, current_owner_record=owner
        )
        active = _active_owner_authorization(owner)
        reference = owner["active_service_authorization"]
        if active is None or reference is None:
            raise RuntimeError("INSTALLER_SERVICE_ACTIVE_AUTHORIZATION")
        active_request = (
            install_request
            if reference["kind"] == "install"
            else recovery_requests[reference["generation"] - 1]
        )
        request_sha256 = hashlib.sha256(
            _canonical_json(active_request)
        ).hexdigest()
        if active["request_sha256"] != request_sha256:
            raise RuntimeError("INSTALLER_SERVICE_ACTIVE_REQUEST_BINDING")
        return {
            "install_request": install_request,
            "install_authorization": owner["install_authorization"],
            "recovery_requests": recovery_requests,
            "recovery_authorizations": tuple(
                owner["recovery_authorizations"]
            ),
            "owner_transition_history": transitions,
            "owner_record": owner,
            "active_authorization": active,
            "active_request": active_request,
        }

    def _validate_response_directory_journal(self) -> None:
        parent_fd = self.protected_response_directory_journal_parent_fd
        expected_parent = self.protected_response_directory_journal_parent_identity
        status = os.fstat(parent_fd)
        observed_parent = {
            "device": status.st_dev,
            "inode": status.st_ino,
        ...
        if observed_parent != expected_parent:
            raise RuntimeError("PROTECTED_RESPONSE_DIRECTORY_JOURNAL_PARENT")
        basename = f"{self.protected_response_operation_token_sha256}.jsonl"
        fd = os.open(
            basename,
            os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW,
            dir_fd=parent_fd,
        ...
        try:
            ...
            if (
                not stat.S_ISREG(before.st_mode)
                or before.st_nlink != 1
                or before.st_uid != self.protected_response_uid
                or before.st_gid != self.protected_response_gid
                or stat.S_IMODE(before.st_mode) != 0o444
            ):
                raise RuntimeError("PROTECTED_RESPONSE_DIRECTORY_JOURNAL_FILE")
            payload = b""
            while True:
                chunk = os.read(fd, 65_536)
                if not chunk:
            ...
            if (
                ...
                raise RuntimeError("PROTECTED_RESPONSE_DIRECTORY_JOURNAL_DRIFT")
        finally:
            os.close(fd)
        records = decode_exact_response_directory_journal(
            payload,
            operation_token_sha256=(
                self.protected_response_operation_token_sha256
            ),
        )
        if (
            len(records) != 2
            or records[-1]["phase"] != "CREATED"
            or records[-1]["operation_identity"]
            != self.protected_response_parent_identity
        ):
            raise RuntimeError("PROTECTED_RESPONSE_DIRECTORY_JOURNAL_STATE")

    def _read_response_artifact(self, request_id: str) -> bytes:
        self._validate_response_directory_journal()
        return read_protected_response_artifact(
            self.protected_response_parent_fd,
            request_id=request_id,
            expected_parent_identity=self.protected_response_parent_identity,
            expected_uid=self.protected_response_uid,
            expected_gid=self.protected_response_gid,
        )

    def validate_received_response_artifact(
        self,
        ...
        *,
        ...
        request = decode_exact_installer_request(decision.request_bytes)
        if fault_hook is not None:
            fault_hook("before_protected_response_artifact_validation")
        persisted = self._read_response_artifact(str(request["request_id"]))
        if (
            persisted != response_bytes
            or hashlib.sha256(persisted).hexdigest() != response_sha256
        ):
            raise RuntimeError("PROTECTED_RESPONSE_WIRE_BINDING")
        validate_durable_installer_response(
            response_bytes=persisted,
            request=request,
        )
        if fault_hook is not None:
            fault_hook("after_protected_response_artifact_validation")

    def commit_received_response_transition(
        self,
        decision: "WireDecision",
        reply: InstallerReply,
        *,
        ...
        """Build one owner replacement for all response-driven state.

        The caller still owns the handed-off transaction-lock OFD.  This
        method never fsyncs or closes it; `persist_received_reply` fsyncs the
        replacement and its parent before releasing that OFD.
        """
        before = self.read_owner_record_mapping()
        active = self.validate_decision_against_active_authorization(
            decision, before
        )
        after = json.loads(_canonical_json(before))
        if fault_hook is not None:
            fault_hook("before_result_append")
        if decision.authorization_kind == "recovery":
            result = encode_recovery_result(
                recovery_authorization=active,
                terminal_response_sha256=reply.response_sha256,
                terminal_phase=reply.payload["terminal_phase"],
                transaction_lock_identity_sha256=(
                    self.transaction_lock_identity_sha256
                ),
                previous_result=(
                    None if not after["recovery_results"]
                    else after["recovery_results"][-1]
                ),
            )
            after["recovery_results"] = [*after["recovery_results"], result]
        if fault_hook is not None:
            fault_hook("after_result_append")

        # Clearing or switching the active reference is an authorization
        # transition even though it does not create a new authorization.
        after["authorization_sequence"] = before["authorization_sequence"] + 1
        if reply.status == "NO_JOURNAL_NO_MUTATION":
            if decision.authorization_kind != "recovery":
                raise RuntimeError("INSTALLER_NO_JOURNAL_AUTHORIZATION_KIND")
            if fault_hook is not None:
                fault_hook("before_install_reactivation")
            after["active_service_authorization"] = {
                "kind": "install", "generation": 0,
            }
            after["active_authorization_sha256"] = after[
                "install_authorization"
            ]["authorization_sha256"]
            after["pending_action"] = after["install_authorization"][
                "operation"
            ]
            after["pending_local_transition"] = None
            self.append_no_journal_reactivation_transition(
                before_owner=before,
                after_owner=after,
                active_recovery_authorization=active,
                fault_hook=fault_hook,
            )
            if fault_hook is not None:
                fault_hook("after_install_reactivation")
        elif reply.status in {"INSTALLED", "RECOVERED"}:
            after["active_service_authorization"] = None
            after["active_authorization_sha256"] = None
            after["pending_action"] = None
            after["pending_local_transition"] = "VALIDATE_PUBLICATION"
            after["component_hashes"] = reply.payload["component_hashes"]
            after["final_tree_sha256"] = reply.payload["final_tree_sha256"]
            after["receipt_sha256"] = reply.payload["receipt_sha256"]
            after["terminal_response_status"] = reply.status
        elif reply.status == "ABORTED_BEFORE_PREPARED":
            after["active_service_authorization"] = None
            after["active_authorization_sha256"] = None
            after["pending_action"] = None
            after["pending_local_transition"] = "MIRROR_ABORTED_ATTEMPT"
            after["terminal_response_status"] = reply.status
        else:
            raise RuntimeError("INSTALLER_RESPONSE_STATUS")
        after["transport_uncertain"] = False
        after["response_uncertain"] = False
        after["last_error"] = None
        self.replace_owner_record(after, fault_hook=fault_hook)

    def read_required_response_artifacts(
        self,
        *,
        journal_payload: bytes,
        install_request: Mapping[str, object],
        ...
        journal_records = [
            json.loads(line) for line in journal_payload.splitlines()
            if line
        ]
        if not journal_records or any(
            record.get("response_operation_identity")
            != _stable_publication_identity(
                self.protected_response_parent_identity
            )
            for record in journal_records
        ):
            raise RuntimeError("PROTECTED_RESPONSE_OPERATION_IDENTITY")
        required = {
            str(record["actor_request_id"])
            for record in journal_records
            if record.get("terminal_response_sha256") is not None
        }
        required.update(
            str(record["adopted_install_request_id"])
            for record in journal_records
            if record.get("adopted_install_response_sha256") is not None
        )
        ...
        for result in recovery_results:
            request = request_by_generation.get(result["recovery_generation"])
            if request is None:
                raise RuntimeError("PROTECTED_RESPONSE_RESULT_REQUEST")
            required.add(str(request["request_id"]))
        known = {
            str(request["request_id"])
            for request in (install_request, *recovery_requests)
        }
        if not required <= known:
            raise RuntimeError("PROTECTED_RESPONSE_REQUEST_SET")
        expected_entries = {
            f"{hashlib.sha256(request_id.encode()).hexdigest()}.bin"
            for request_id in required
        }
        observed_entries = set(os.listdir(self.protected_response_parent_fd))
        if observed_entries != expected_entries:
            raise RuntimeError("PROTECTED_RESPONSE_ARTIFACT_NAMESPACE")
        return {
            request_id: self._read_response_artifact(request_id)
            for request_id in required
        }

    def validate_publication_before_mirror(
        self,
        *,
        journal_payload: bytes,
        receipt_payload: bytes,
        ...
        artifacts = self.read_required_response_artifacts(
            journal_payload=journal_payload,
            install_request=install_request,
            recovery_requests=recovery_requests,
            recovery_results=recovery_results,
        )
        return validate_protected_publication_contract(
            journal_payload=journal_payload,
            ...
            recovery_results=recovery_results,
            owner_transition_history=owner_transition_history,
            current_owner_record=current_owner_record,
        )


# The production declaration is:
# class DiskAuthorizationStore(
#     OwnerTransitionTransactionMixin, ProtectedResponseValidationMixin
# ): ...
@dataclass(frozen=True, slots=True)
class WireDecision:
    request_bytes: bytes
...
class InstallerManagerOwnership:
    ...
    def restart(cls, root: Path) -> "InstallerManagerOwnership":
        store = DiskAuthorizationStore.open_unreconciled(root)
        construction_lock_fd = store.open_and_lock_construction()
        try:
            store.validate_locked_construction_fd(construction_lock_fd)
            store.reconcile_owner_transition_transaction()
            return cls(
                store=store,
                construction_lock_fd=construction_lock_fd,
            )
        except BaseException:
            os.close(construction_lock_fd)
            raise
    ...
    def persist_received_reply(
        ...
        *,
        ...
        try:
            self.store.validate_received_response_artifact(
                decision,
                response_bytes=reply.response_bytes,
                response_sha256=reply.response_sha256,
                fault_hook=fault_hook,
            )
            # This single owner replacement includes the result append and
            # the active-reference clear/switch.  In particular,
            # NO_JOURNAL_NO_MUTATION cannot leave REACTIVATE_INSTALL for a
            # later, unlocked transition.
            self.store.commit_received_response_transition(
                decision,
                reply,
                fault_hook=fault_hook,
            )
    ...
    def reconcile_locked_handoff(self) -> None:
        ...
        if retained.reply is not None:
            self.store.validate_received_response_artifact(
                retained.decision,
                response_bytes=retained.reply.response_bytes,
                response_sha256=retained.reply.response_sha256,
            )
            self.store.reconcile_received_response_transition(
                retained.decision, retained.reply
            )
        else:
    ...
    def begin_install(
        ...
        *,
        ...
        try:
            ...
            self.store.commit_transport_uncertain_transition(
                decision,
                error_code=str(error)[:128],
                fault_hook=fault_hook,
            )
            if fault_hook is not None:
                fault_hook("after_transport_uncertain_transition")
    ...
    def _complete_pending_local_transition(
        ...
        *,
        ...
        if pending == "MIRROR_ABORTED_ATTEMPT":
            if fault_hook is not None:
                fault_hook("before_old_attempt_abort")
            self.store.mirror_aborted_attempt_before_prepared(
                fault_hook=fault_hook,
                boundary_prefix="old_attempt_abort",
            )
            self.store.fsync_owner_and_parent(
                fault_hook=fault_hook,
                boundary_prefix="old_attempt_abort",
            ...
            if fault_hook is not None:
                fault_hook("after_old_attempt_abort")
            return True
        if pending == "ALLOCATE_FRESH_BASE_ATTEMPT":
            if fault_hook is not None:
                fault_hook("before_fresh_base_allocate")
            self.store.allocate_fresh_base_attempt(
                fault_hook=fault_hook,
                boundary_prefix="fresh_base_allocate",
            )
            self.store.fsync_owner_and_parent(
                fault_hook=fault_hook,
                boundary_prefix="fresh_base_allocate",
            ...
            if fault_hook is not None:
                fault_hook("after_fresh_base_allocate")
            return True
        if pending == "REBUILD_FRESH_BASE":
            ...
            self.store.resume_fresh_base_copy_to_base_copied(
                fault_hook=fault_hook,
                boundary_prefix="fresh_base_copy",
            )
            self.store.fsync_owner_and_parent(
                fault_hook=fault_hook,
                boundary_prefix="fresh_base_copy",
            ...
            if fault_hook is not None:
                fault_hook("after_fresh_base_copy")
            return True
        if pending == "ACTIVATE_FRESH_INSTALL":
            if fault_hook is not None:
                fault_hook("before_fresh_install_authorize")
            self.store.activate_fresh_install_authorization(
                fault_hook=fault_hook,
                boundary_prefix="fresh_install_authorize",
            )
            self.store.fsync_owner_and_parent(
                fault_hook=fault_hook,
                boundary_prefix="fresh_install_authorize",
            ...
            if fault_hook is not None:
                fault_hook("after_fresh_install_authorize")
            return True
...
`DiskAuthorizationStore` is not an in-memory adapter. It exclusively creates canonical `install-request.bin`, append-only authorization/result JSONL, an independent owner-transition history, an authoritative append-only `owner-transition-transactions.jsonl`, append-only locked-response uncertainty records, a separate append-only `preacquisition-diagnostics.jsonl`, and an atomically replaced `owner-record.json` beneath a mode-`0700` manager root; every record and replacement is fsynced with its parent before its reference becomes active. A transaction-busy diagnostic never replaces or overlays the owner record used by the privileged service. Every logical owner/history update is committed through `OwnerTransitionTransactionMixin`: a canonical `PREPARED` frame containing the complete nested transition entry and exact before/after owner digests is durably published first; the history candidate with the unchanged prefix plus that entry is atomically replaced and fsynced; the exact after-owner is atomically replaced and fsynced; and only then is a hash-linked `COMMITTED` frame durably appended. Bootstrap uses this same protocol for the initial install-authorization transition; an empty transaction journal is legal only with an empty transition history and the exact preauthorization owner record. `DiskAuthorizationStore.open_unreconciled()` is strictly non-mutating: it pins and validates manager-root paths and lock identities but neither repairs nor replaces a file. `InstallerManagerOwnership.restart()` must then acquire and validate the stable construction-lock OFD before calling `reconcile_owner_transition_transaction()`. Under that lock, a trailing `PREPARED` may have history equal either to the committed prefix or that prefix plus the prepared entry, and owner bytes may equal only the prepared before or after bytes; reconciliation completes the missing steps idempotently and emits `COMMITTED`. Every other combination is corruption, and every exception closes the construction FD without another mutation. `open_readonly()` never reconciles and refuses a pending transaction. The privileged service calls `read_committed_owner_transition_history()` and refuses an uncommitted tail. Before the privileged installer creates an operation-scoped response directory, it pins the configured response-root FD and records its exact identity in `PENDING` under the separate root-owned `response-directory-journals/<operation-token>.jsonl`. On every replay it opens and authenticates that root against `PENDING` before any child mutation, performs exclusive FD-relative `mkdir`, validates the empty child through a no-follow directory FD, fsyncs the pinned response-root FD, and appends/fsyncs `CREATED` with the exact child device/inode/mount/UID/GID identity. An existing directory without that legal journal prefix is never adopted, and a swapped/drifted response root receives no new entry. Exact canonical response bytes are written into a request-and-payload-derived mode-`0600` temporary inode inside that authenticated operation directory, fsynced completely, changed to `0444`, metadata-fsynced, then atomically published as `<sha256(request_id)>.bin` with `renameat2(RENAME_NOREPLACE)` and followed by an operation-directory fsync before any terminal publication-journal append, result, or send. The final pathname is never created by `open(O_EXCL)` and can therefore never expose empty or partial contents. Recovery may discard only the exact unpublished token-owned temporary inode; a fully published final inode is adopted only after a no-follow reopen proves the directory-journal identity, regular-file type, link count one, configured UID/GID, mode `0444`, stable metadata, exact request ID, canonical bytes, and byte identity with the response about to be sent. A byte-identical install replay first detects the existing install-token journal, decodes its exact complete install-only phase sequence, validates the already-published root, components, completion metadata, receipt, operation-directory identity, and canonical response artifact, and returns those existing identities. It must not create staging, create/append a journal or receipt, replace a response artifact, chmod, rename, or fsync a changed namespace; replay tests require a byte/inode/mode snapshot of the entire protected-installer tree to remain identical. A partial install journal is not eligible for install replay and requires authenticated recovery. A crash after directory `PENDING`, after `mkdir`, after response-root fsync, after `CREATED`, or after response rename is replayed from the exact durable phase. Existing final artifacts are never overwritten or deleted. Before every publication mirror, `validate_received_response_artifact()` pins both the directory-journal parent and operation directory, validates the exact two-record journal, enumerates every operation entry, and requires exact equality with request-ID basenames derived from all terminal journal records, recovery adoption records, and consumed recovery results. Pending, extra, missing, symlinked, or foreign names fail closed. `OWNER_RECORD_KEYS` is the exact persisted schema, not a reduced authorization view: it includes the complete construction identity and attempt state, both tokens, owner process identity, paths, phase and pending state, cleanup and uncertainty state, immutable install authorization, complete recovery authorization/result histories, active reference and `active_authorization_sha256`, authorization sequence, protected-validation mirrors, component/final/receipt identities, diagnostics, and terminal state. Install and recovery authorizations use distinct exact schemas. Both persist operation, complete construction identity/attempt/token fields, canonical request digest, sequence, authorization digest, and owner transition; recovery additionally persists generation, fresh recovery-token digest, reason, and `previous_entry_sha256`, which is null for generation 1 and otherwise hashes the complete preceding canonical recovery-authorization entry—not the install authorization and not its inner authorization digest. Recovery results have their own canonical `result_sha256`, form a contiguous generation prefix, bind the corresponding authorization/request and protected response artifact digest, and hash-link only to the preceding result. `read_owner_record_bytes()` reopens the current owner path with no-follow/beneath semantics, requires a canonical regular-file record, and returns the exact bytes whose decoded object is also exposed by `validated_snapshot`; `read_service_evidence()` independently reopens those bytes, all canonical request files, the committed owner-transition transaction journal, and the nested owner-transition history only after the service acquires `transaction.lock`. Every `owner_transition_history` entry has exactly the nested `{transition, from_owner_record, to_owner_record}` representation in memory, durable history, manager snapshots, and privileged-decoder input. `OwnerTransitionEntry.from_mapping()` and `.as_mapping()` are the only object/mapping conversion seam; flattened transition fields and caller-supplied resume snapshots are forbidden.

Install replay uses a dedicated read-only response-evidence decoder, never the crash-recovery writers. It opens the configured response root and directory-journal parent without following links; requires the exact immutable `PENDING, CREATED` directory-journal sequence and recorded root/operation identities; enumerates exactly one request-ID-derived response artifact; requires a regular single-link configured-owner mode-`0444` inode on the recorded mount; reads stable canonical bytes; and binds their digest to the install publication journal's `terminal_response_sha256`. The replay branch sends those stored bytes directly. It cannot call `_ensure_response_operation_directory`, `_record_terminal_response`, `_write_install_publication_journal`, or any helper that creates, appends, repairs, chmods, renames, or fsyncs protected evidence. A missing or extra artifact, changed metadata or bytes, and a `PENDING`-only directory journal all fail before send while the full protected-tree byte/inode/mode snapshot remains unchanged.

Authorization construction is explicitly acyclic. For a newly created authorization, the transition intent hashes the exact prior canonical owner-record bytes plus the complete intended target owner record with only the new authorization's own `authorization_sha256`, its embedded `owner_transition`, and the top-level `active_authorization_sha256` neutralized to null. Every other security-relevant field remains in that projection. The authorization digest then binds the transition intent and exact request digest; only afterward is the same target record encoded with the new authorization and identical digest at both the active object and top-level active-digest field. Reactivation creates no authorization, so its projection is the complete intended target record without neutralization. The append-only transition evidence stores the exact canonical before/after owner objects beside the intent, and `validate_owner_transition_against_records` must recompute the prior full-record digest, target projection digest, full field-for-field owner delta, active authorization object and digest, active IDs/sequences/actions, authorization digest, and transition chain from those persisted bytes. Consecutive authorization entries may differ only through two explicitly validated omitted-update reasons: nondecreasing heartbeat fields, or a phase-only forward transition within the phase table for the unchanged construction kind. `_validate_permitted_owner_transition_gap` compares every owner key and classifies exactly one reason. Transaction-busy evidence is stored outside the owner record. Cross-kind or backward phases fail explicitly, while state, attempt, cleanup, pending, uncertainty, error, locked-response, authorization/result/history, active-reference, publication-mirror, identity, path, and token changes remain forbidden. Those other lifecycle and response-result transitions are validated by their owning durable state machines and may not hide in an authorization-history gap. While a privileged service holds its transaction, `validate_owner_history_for_service` is stricter: from the exact target of the authenticated authorization transition to the independently reopened current owner, only monotonic heartbeat fields may differ. Phase progress, cleanup, pending state, diagnostics, or any other owner replacement is rejected until the service transaction ends. Transition SHA/sequence and source authorization ID/sequence chains remain exact. A target full-owner digest is never an authorization input. The durable store, installer service, and tests all call the same owner-record, authorization, transition, gap-reason, and service-tail validators; fixtures may not invent owner hashes or reduced owner objects. Tests bootstrap the real `DiskAuthorizationStore`, pass its actual nested transition entries through the privileged decoder, accept inter-authorization heartbeat and same-kind phase gaps, accept only a final service heartbeat tail, and reject busy-diagnostic owner changes, final service phase changes, heartbeat rollback, cross-kind `BASE_COPIED -> HSSD_NORMALIZED`, cleanup/pending/uncertainty gaps, plus independent security-field mutations.

`replace_owner_transition_transaction_bytes`, `replace_owner_transition_history`, and `replace_owner_record_bytes` each write the complete candidate to a same-directory mode-`0600` temporary inode, check the full write, fsync the file, atomically rename without following links, and fsync the pinned parent FD. They expose `before_` and `after_` hooks for `replace`, `file_fsync`, and `parent_fsync` under the exact prefixes `owner_transition_prepared`, `owner_transition_history`, `owner_transition_owner`, and `owner_transition_committed`. No history or owner mutation occurs until the PREPARED parent fsync has completed; COMMITTED is unreachable until both the history and owner parent fsyncs have completed. A fork/`_exit` matrix covers all 24 hooks for both install-to-recovery uncertainty activation and same-generation recovery replay, then invokes `reconcile_owner_transition_transaction()` from disk and requires the committed history, current owner, and service evidence to agree before continuing with a manager-state-backed replacement service.

The root manifest pins the construction- and transaction-lock device/inode/mount identities, the configured protected journal/receipt/publication/response-artifact parent identities, the response-artifact UID/GID/mode policy, and the original install digest. The store calls only `encode_installer_request` for install and recovery documents. Every generated wire document therefore has exactly the six top-level request keys, keeps construction-attempt and recovery-generation fields inside the exact operation payload, and records an explicit request ID that byte-identical replay preserves. `prepare_next_wire_request` places the authorization-append and active-reference fault hooks around their actual durable operations and returns a `WireDecision` containing the exact bytes that the session will send. `InstallerOrchestrator._exchange` alone owns the request-delivery and terminal-frame-plus-handoff hooks, passing its still-locked construction FD to `UnixInstallerSession`; the session retains the exact response bytes alongside their digest. After the privileged service acquires its root transaction lock and reopens the current owner record, it opens the token-derived authorization, result, and transition JSONL files beneath the fixed configured manager-control parent with no-follow/beneath semantics. It accepts only full transition entries containing canonical before/after owner objects and calls `validate_owner_history_for_service`, which authenticates the complete history and heartbeat-only final tail to the independently reopened current owner; a caller-supplied transition dictionary is never an authorization input. The store opens protected evidence read-only and calls only `validate_publication_before_mirror`, which derives the exact required response-artifact set as the union of (a) request IDs in every protected terminal journal record and (b) request IDs corresponding to every consumed recovery result. Thus `NO_JOURNAL_NO_MUTATION` is mandatory evidence even though it intentionally has no publication-journal entry; an unjournaled result is legal only for that exact status and terminal phase. An artifact durable before its journal/result binding is adopted during byte-identical request replay but remains unauthorized for a mirror until that binding is durably committed. The method passes those exact artifacts, the immutable original install request, complete authorization/result/transition histories, canonical current owner, and journal/receipt bytes to `validate_protected_publication_contract`. The shared wrapper runs the structural journal/receipt decoders, response codec, canonical recovery-result validator, full owner-transition-history validator, and `validate_current_owner_for_publication`. The latter compares every owner field against the final authenticated transition target, permits only the exact phase-specific response/publication state plus monotonic heartbeats, and explicitly binds tokens, paths, construction identity, attempt, cleanup, pending state, histories, active reference, uncertainty, mirrors, and final hashes. Only then may the first or later mirror occur. Direct low-level journal decoding cannot authorize a publication mirror. The response digest is excluded from the receipt, so atomically publishing and fsyncing the response artifact after receipt creation and binding its digest into the final journal record creates no hash cycle.

`persist_received_reply` places result-append, active-reference, owner-replacement/fsync, and handoff-close hooks around every valid `INSTALLED`, null-recovery, aborted, or `RECOVERED` reply. It constructs exactly one response-driven owner replacement while the handed-off transaction OFD remains locked. Every clear or switch of `active_service_authorization` increments `authorization_sequence` by exactly one. `INSTALLED` and `RECOVERED` durably record the wire response, clear the active service authorization, increment the sequence, leave the owner phase at `INSTALLING` (or `BASE_METADATA_NORMALIZED`), set `pending_local_transition="VALIDATE_PUBLICATION"`, keep `cleanup_required=true`, and remain nonterminal. `NO_JOURNAL_NO_MUTATION` atomically appends its recovery result and selects the unchanged install authorization plus byte-identical request in that same replacement; there is no later `REACTIVATE_INSTALL` state. Only after that owner file and its parent are fsynced may the transaction OFD close. `ABORTED_BEFORE_PREPARED` appends its result, clears the active reference with the same sequence increment, and sets `pending_local_transition="MIRROR_ABORTED_ATTEMPT"` in that same locked replacement. Four restartable transitions then run separately: (1) validate the exact `INITIAL → RECOVERY_STARTED → STAGING_REMOVE_PENDING → ABORTED_BEFORE_PREPARED` journal with null publication fields, fsync `attempt_status="aborted-before-prepared"`, and set `ALLOCATE_FRESH_BASE_ATTEMPT`; (2) increment the attempt, generate a fresh operation token, fsync `attempt_status="active"`, `phase="BASE_ALLOCATED"`, and `REBUILD_FRESH_BASE`; (3) resume the normal write-ahead base-copy operation and separately fsync `phase="BASE_COPIED"` plus `ACTIVATE_FRESH_INSTALL`; and only then (4) create/fsync the new immutable install authorization, empty recovery histories, active install reference, and exact fresh install request. No method performs two of these owner-record replacements. Each method and `fsync_owner_and_parent` accepts a transition-specific fault-hook prefix and exposes eight ordered boundaries: before the transition, before/after atomic owner replacement, before/after owner-file fsync, before/after parent-directory fsync, and after the transition. Fork/`_exit` tests cover all 32 boundaries, reopen the real store, require exactly the prior or new canonical state appropriate to that boundary, and then finish from disk without fabricating an intermediate object.

`advance_publication_validation()` is driven only by the last fsynced owner phase and performs exactly one durable owner-phase transition per call. From `INSTALLING`/`BASE_METADATA_NORMALIZED`, it no-follow opens the token-derived protected journal, receipt, and configured response-artifact parent and invokes the production `validate_publication_before_mirror` seam. That seam derives and reads the union of terminal-journal and consumed-result artifacts, freshly reopens the current owner, and calls the shared full contract; it must return successfully before the `before_final_renamed_mirror` hook or the first owner replacement is reachable. The manager then atomically mirrors `FINAL_RENAMED`/`BASE_FINAL_RENAMED` and separately fsyncs owner and parent. From that fsynced phase it repeats the whole shared contract, atomically mirrors `RECEIPT_CREATED`/`BASE_RECEIPT_CREATED`, and separately fsyncs owner and parent. From that fsynced phase it repeats the whole shared contract, no-follow reopens the configured digest-derived published root, validates its device/inode/mount/UID/GID/mode, component manifests, completion metadata, and receipt binding, then atomically enters `COMPLETE`/`BASE_COMPLETE`, sets `cleanup_required=false`, clears the pending transition, and separately fsyncs owner and parent. Validation-only progress is never stored, so a crash at a post-handoff validation hook repeats validation from the last durable phase without a wire request. In contrast, a crash at `before/after_protected_response_artifact_validation` occurs before the recovery result and owner transition are committed: restart must replay the byte-identical authenticated request, and the service must adopt the immutable artifact. Tests maintain distinct receive-time and post-handoff boundary sets and assert two requests versus one respectively. Every invocation refuses extra paths, response artifacts, suffix records, symlinks, or values learned only from the wire.

`commit_locked_response_uncertain` appends a canonical, independently hash-chained locked-response record containing the decision request digest, active authorization digest/generation, received-prefix digest, bounded error code, and transaction-lock identity while the valid transaction-lock OFD is still held. A malformed install response uses the explicit `activate_recovery_after_locked_install_response` owner transition: the same logical before/after entry appends exactly one audit record, creates and activates exactly the next recovery authorization, increments the authorization sequence once, and leaves `response_uncertain=true`. A malformed response to an already-active recovery uses `append_locked_recovery_response_audit`: it appends exactly one audit record while retaining the same recovery generation, request bytes, active reference/digest, authorization sequence, phase, and pending action. That audit transition advances only transition sequence, not authorization sequence. Missing, invalid, truncated, or otherwise unauthenticated handoff evidence cannot use the handed-off-lock path. While still owning the stable construction lock, an install uncertainty uses `activate_recovery_after_transport_uncertain_install`: it creates and activates exactly the next recovery authorization, increments authorization sequence once, records bounded `last_error`, sets only `transport_uncertain=true`, and changes no phase, cleanup, publication, result, locked-response, path, or token field. Transport uncertainty for an already-active recovery uses `record_transport_uncertain_recovery_replay`: it keeps the same authorization object/generation/digest, request bytes, authorization sequence, phase, and pending action while recording only bounded transport status for byte-identical replay. The latter advances transition sequence but not authorization sequence. All four reason-specific validators reject every unlisted delta. Every one of these logical owner/history changes commits through the PREPARED/history/owner/COMMITTED transaction protocol; a valid handed-off OFD remains held through COMMITTED, while a stable-lock transport transition remains under the construction lock. Consequently, a replacement service's independently reopened committed history and current owner differ only by heartbeat. Locked-response records are immutable audit history: subsequent recovery, reactivation, publication validation, and `COMPLETE` retain the exact prefix and validate every record against the complete authorization request set; they are never reset to `[]`. A strictly decoded `INSTALLER_TRANSACTION_BUSY` is different: it has zero FDs, exact retryable error semantics and all mutation-result fields null. `record_preacquisition_failure` appends only an fsynced diagnostic; it does not change the active authorization/reference, request bytes, phase, pending action, construction attempt, recovery histories, or cleanup state. A retry sends byte-identical bytes. Every continuation fixture passes only `manager_state_root`; after acquiring `transaction.lock`, it reopens committed transaction/history evidence and the current owner instead of using synthetic resume state.

Reactivation, publication-validation, active-reference-clear, and resubmission hooks surround those real orchestration operations, never a synthetic reply. The response-driven reactivation and clear hooks execute inside `commit_received_response_transition`, before the handed-off OFD is released; there is no unlocked follow-up state. `validated_snapshot` exposes non-null `transport_uncertain`, `response_uncertain`, last pre-acquisition status, active authorization kind/generation, locked-response records, protected-validation progress, and exact next/last request bytes; `recovery_decision`, `reconcile_received_response_transition`, `reconcile_locked_response_uncertain`, and restart methods reopen all files with `openat2`/no-follow semantics, reject any invalid suffix or temporary entry, verify both independent hash chains and the active-reference digest, and never infer state from an un-fsynced pending file. Live reconciliation closes neither FD on any error. Restart acquires the pinned construction lock and uses only the fsynced prefix. It reaches terminal only by receiving and durably recording the next required authenticated wire response, if one is still required, and then completing all pending independent publication-validation transitions.

`AuthorizationHistory.append_recovery()` requires generation `last + 1`, a fresh token and authorization ID, `previous_entry_sha256=None` for generation 1 and otherwise the exact canonical digest of the preceding recovery-authorization entry, a monotonic authorization sequence, and one active reference. `RecoveryResultHistory.append()` separately requires a contiguous result prefix and `previous_result_sha256=None` for its first result and otherwise the preceding result digest. Authorization objects contain no result field, and result entries never participate in the authorization chain. `NO_JOURNAL_NO_MUTATION` reactivates the unchanged install authorization and byte-identical request only when the newly appended result has that exact terminal phase and matches the before-record's active final recovery generation, authorization digest, and request digest. `ABORTED_BEFORE_PREPARED`, `BASE_COMPLETE`, `COMPLETE`, or a result for any other generation cannot reactivate install even if every affected object and hash is recomputed coherently. An uncertainty while the install authorization is active appends exactly the next recovery generation; an unauthenticated transport or malformed locked response while a recovery authorization is already active retains and replays that same generation byte-for-byte. A later generation is permitted only after an independently durable recovery result/reactivation cycle has returned authority to the install request and that install request becomes uncertain again. Neither history overwrites or reorders an entry, marks an older entry active, or uses `pending_action` as authority.
...
construct the exact canonical INSTALLED/RECOVERED response bytes -> write and
fsync a request-and-payload-derived temporary response inode -> fchmod(0444) and
fsync metadata -> renameat2(RENAME_NOREPLACE) to the request-ID final basename ->
fsync the response parent (or strictly adopt an exact prior final artifact) -> append/fsync
...
Before `PREPARED`, staging cleanup is implemented only by a confined descriptor walk rooted at the already validated staging FD. The native service enumerates each directory through that FD, opens child directories with no-follow/beneath restrictions, rejects a mount-ID change, unlinks non-directories with `unlinkat`, removes emptied child directories with `unlinkat(..., AT_REMOVEDIR)`, and fsyncs every changed child directory. It then removes the exact token basename through the retained staging-parent FD, emits the `after_exact_staging_rmdir` crash barrier before any parent fsync, fsyncs that already-open configured parent, and emits `after_staging_parent_fsync` before appending `STAGING_REMOVED`. Restart from durable `STAGING_REMOVE_PENDING` accepts either the exact revalidated staging inode or its absence under the unchanged configured parent identity; it repeats/finishes the parent fsync before advancing. It never constructs a child pathname, traverses `..`, or enumerates/removes any sibling. Partial nested content, symlinks, and a staging-local final-looking completion file are removable; fork/`_exit` tests at both new barriers prove restart completion while an unrelated sibling's bytes and inode remain unchanged.

Response artifacts use the same no-follow parent-FD discipline as receipts. A fixed configured response root is disjoint from payload hashes and writable only by the native installer; one exclusively created child directory named by the construction operation-token digest contains every response for that attempt. The protected journal binds that child directory's stable identity, and the manager pins and enumerates that directory—not a mapping synthesized from expected request IDs. The final basename is exactly `sha256(request_id).bin`; the temporary basename is exactly `.<final-basename>.<sha256(canonical-response-bytes)>.pending`. The service exclusively creates the temporary entry at `0600`, performs complete-write checking, fsyncs contents, changes it to `0444`, fsyncs again, no-follow reopens it, then uses FD-relative `renameat2(RENAME_NOREPLACE)` to publish the fully durable inode and fsyncs the operation directory before the terminal journal append, result, or send. The final path is never opened for writing. Recovery distinguishes every boundary: an absent final plus an exact partial temporary is safely removed and rebuilt; an absent final plus a complete immutable temporary is published; a present final after a crash before directory fsync is fully revalidated and followed by an idempotent directory fsync. Any unrelated temporary, final/temp identity ambiguity, or non-exact metadata/bytes is corruption and is preserved for inspection. Recovery from a crash after artifact publication but before a terminal journal record first appends `RECOVERY_STARTED`, reconstructs and validates the exact canonical orphan install response implied by the request plus protected receipt/publication state, adopts that immutable inode, and then appends `RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED` under the active recovery authorization. It never appends a recovery-written install-bound `RECEIPT_CREATED`; manager authorization still requires a byte-identical replay or authenticated recovery and a durable recovery-authored adoption or consumed-result binding. A different inode at the final basename, wrong operation-directory identity, symlink, non-regular file, link count other than one, wrong UID/GID/mode, byte drift, request-ID mismatch, or any extra namespace entry is terminal corruption. `NO_JOURNAL_NO_MUTATION` means no publication-journal or payload mutation; its root-owned response evidence is still created in this disjoint operation directory, selected by the consumed recovery-result chain, and validated as part of the exact artifact union before any later publication mirror.

Receipt adoption uses the configured `input_uid`/`input_gid` and configured receipt-parent device/inode/mount/UID/GID/mode identity, not the service's ambient identity or a mount ID learned from the candidate path. Before appending any recovery record—even when the existing journal already ends in `RECEIPT_CREATED`—the service no-follow opens and validates the configured parent FD, derives the receipt basename from the install-bound digest, and opens the entry through that FD. It requires a regular file, link count exactly one, owner/group equal to configuration, mode `0444`, the configured parent mount ID, exact length and canonical bytes, and stable device/inode/mount/type/link/owner/mode/size before and after reading. Immediately before the recovery append it closes and performs one final no-follow reopen through the same configured parent, requiring the same inode and bytes; successful recovery repeats this final reopen before constructing its response. Only `ENOENT` at a journal phase that legally precedes receipt creation permits `O_CREAT|O_EXCL`; a symlink, directory, hard link, wrong owner/group/mode, cross-mount object, byte mismatch, metadata drift, or changed parent identity blocks recovery without replacement or journal append.
...
construction; before/after response temporary O_EXCL; during/after temporary write;
after temporary data fsync; after chmod; after metadata fsync; before/after
renameat2(RENAME_NOREPLACE); after response-parent fsync;
before terminal-response-digest append; after its journal fsync;
after RECEIPT_CREATED/RECOVERY_RESPONSE_CREATED; before sendmsg; after sendmsg;
after exact pre-PREPARED staging rmdir; after staging-parent fsync;
before local FD close; during shutdown
```

For each point assert a restart produces either no final path or one manifest-valid immutable final path with exactly one byte-identical receipt. Response tests additionally assert that no crash exposes a partial final artifact, that the exact temporary entry is removed or adopted according to its durable phase, the final inode ends at link count one, the response parent is fsynced, and an unrelated sibling is unchanged. Drive every case through the six-field canonical request transport with one already-locked construction FD. Add one-mutation-at-a-time rejection cases for missing/extra/wrong FDs, ancillary truncation, wrong `fd_roles`, generation fields outside `payload`, noncanonical JSON, an echoed request-ID mismatch, a valid-lowercase-but-incorrect authorization digest, and deletion/change of any immutable owner-transition entry; every rejection must precede journal access or the next mutation. Persist the full actual `DiskAuthorizationStore` owner objects before and after each transition, recompute the acyclic complete-target projection and authorization digest from those bytes, assert `after.active_authorization_sha256` equals the activated authorization's recomputed digest, and independently mutate construction attempt, owner/operation tokens, phase, cleanup state, recovery authorizations/results, active reference, and active digest. The history matrix must separately reject gapped and reordered recovery authorization generations, install/recovery cross-kind entries, broken canonical-authorization links, non-prefix/gapped results, reordered results, and broken previous-result or own-result digests. It must also build coherently rehashed transitions whose before/after objects change exactly one forbidden token, path, phase, cleanup, or history field; generic hashes must pass and reason-specific owner-delta validation must reject them. Add the mandatory generation-1 no-journal → identical install replay → post-journal uncertainty → generation-2 recovery sentinel and assert generation 1 plus the intervening install-reactivation transition remain in the independently verified histories even though neither is a publication-journal actor. The final shared-contract call must require response artifacts for both the consumed no-journal generation and all terminal journal actors, reject either omission or any extra artifact, and preserve the no-journal artifact's bytes/hash/inode across replay. Add the `INITIAL`-only recovery matrix at `RECOVERY_STARTED`, `STAGING_REMOVE_PENDING`, after the exact staging-root `rmdir`, after its parent fsync, and `ABORTED_BEFORE_PREPARED`; seed nested partial/final-looking files plus symlinks and prove restart removes only the target while preserving an unrelated sibling's bytes and inode. After abort, fork/`_exit` before the transition, before/after owner replacement, before/after owner-file fsync, before/after parent-directory fsync, and after the transition for each of old-attempt abort, fresh `BASE_ALLOCATED`, fresh `BASE_COPIED`, and new install authorization. Add the receipt-fsync-before-journal-append boundary and require recovery to adopt only a byte-identical receipt while preserving bytes, hash, inode, owner, mode, regular-file type, and link count. Starting from a journal already complete through `RECEIPT_CREATED`, independently mutate configured parent mount identity and receipt UID/GID, mode, type, and link count; each case must fail before `RECOVERY_STARTED`, preserve the journal bytes, and perform no replacement. Run two concurrent installers and two concurrent recoverers; require one mutation winner and zero signals/deletions by losers.
...
The install-replay rejection matrix begins with a complete protected install, then independently removes the request artifact, adds a foreign artifact, mutates the canonical artifact bytes, or truncates the response-directory journal to durable `PENDING`. Each case first invokes the replay validator directly and then traverses the real replacement-service socket path. A second six-case family sends two byte-identical install requests to one still-running service: the first creates the valid publication and leaves its publication object cached; the test then applies the same four response-evidence corruptions, deletes only the install journal, or deletes both the journal and request artifact. An existing journal must force strict replay, while either journal-deletion case must reject the populated-cache/absent-journal state before the artifact recorder or journal writer is reachable. All calls must fail before send/publication, and the entire protected-installer tree snapshot—including every relative name, inode, mode, file digest, and size—must equal the pre-call corrupt snapshot. The same-service family also requires the request log to contain exactly the two byte-identical canonical requests. No case may complete `CREATED`, recreate a missing journal or artifact, remove the extra artifact, restore bytes, or call a protected-directory fsync.
...
The response-directory crash matrix additionally requires `PENDING` before exclusive operation-directory creation and `CREATED` only after the response-root fsync; a missing, malformed, or complete-looking unjournaled directory fails without deletion, and the preserved sibling belongs to a different operation token. Recovery after an install crash at `FINAL_RENAMED` preserves the install prefix byte-for-byte, adds no recovery-written `RECEIPT_CREATED`, and appends a recovery-authored `RECOVERY_ORPHAN_INSTALL_RESPONSE_ADOPTED` record binding the original install request ID and exact orphan response digest before recovery validation and response.
